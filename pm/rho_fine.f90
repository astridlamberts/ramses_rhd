...
subroutine rho_fine(ilevel,icount)
  ...
  if(m_refine(ilevel)>-1.0d0)then
     ...
     ! Prevent from refining too much relative to available star mass resolution
     ! This translates into a constant physical resolution, except at very high z.
     if(star)then
        ! Finest cell size
        ...
        d0   = nISM/scale_nH
        ! Star particle mass
        mstar=MAX(del_star*omega_b*rhoc*X/mH,n_star)/(scale_nH*aexp**3)*vol_min
        ! Test is designed so that nlevelmax is activated at aexp \simeq 0.8
        if(d0*(dx_loc/2.0)**ndim.lt.mstar/2d0)m_refine_loc=1d10 
     endif
