...
!!$           d_thres=d_jeans
           ...
           msink_new(index_sink)=(d-d_thres/4.0)*dx_loc**3
           ...
           uold(ind_cell_new(i),1)=d_thres/4.0
...
subroutine grow_bondi(ilevel)
  ...
  do isink=1,nsink
     ...
     dMBHoverdt(isink)=4.*3.1415926*density*min(r2sink(isink),r2k(isink)) &
          & *sqrt(c2sink(isink)+v2sink(isink))
  end do
