...
subroutine star_formation(ilevel)
  ...
  if(eta_sn>0.and.ndebris>0)then
  ...
  ! Long lived star particle mass
  mstar=MAX(del_star*omega_b*rhoc*XH/mH,n_star)/(scale_nH*aexp**3)*vol_min/mass_load
  dstar=mstar/vol_loc
     ...
     do ind=1,twotondim
        ...
        do i=1,ngrid
           ...
           if(ok(i))then
              ...
              ! Compute total depleted gas
              dgas=nstar(i)*dstar*mass_load
              ! Security to prevent more than 90% of gas depletion
              if (dgas > 0.9*d) then
                 nstar_corrected=int(0.9*d/(dstar*mass_load))
              ...
              if(eta_sn>0.and.ndebris>0)ndebris_tot=ndebris_tot+2*ndebris*nstar(i)
           endif
        enddo
     ...
     do ind=1,twotondim
        ...
        do i=1,nnew
           ...
           if(eta_sn>0.and.ndebris>0)then
              ...
              ! No debris case
              mp(ind_part(i))=mp(ind_part(i))*mass_load
              idp(ind_part(i))=-idp(ind_part(i))
           endif
        ...
        do i=1,ngrid
           uold(ind_cell(i),1)=uold(ind_cell(i),1) - flag2(ind_cell(i))*dstar*mass_load
        end do
     end do
     ! End loop over cells
  end do
