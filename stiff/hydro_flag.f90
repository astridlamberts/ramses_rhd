  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        do idim=1,ndim
           ...
           ! Convert hydro variables from code units to user units
           do ivar=1,nvar
              if(ivar==1)then
                 scale_var=1.0
              else if (ivar<ndim+2)then
                 scale_var=scale
              else if (ivar==ndim+2.or.ivar==ndim+4)then
                 scale_var=scale**2
              else
                 scale_var=1.0
              end if
              do i=1,ngrid
                 uug(i,ivar)=uold(indn(i,2*idim-1),ivar)/scale_var
                 uum(i,ivar)=uold(ind_cell(i     ),ivar)/scale_var
                 uud(i,ivar)=uold(indn(i,2*idim  ),ivar)/scale_var
              end do
           end do
