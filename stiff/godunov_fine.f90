...
subroutine set_unew(ilevel)
  ...
  ! the hydro scheme. unew is set to zero in virtual boundries.
  !--------------------------------------------------------------------------
  ...
  do ind=1,twotondim
     ...
     do ivar=1,nvar
        do i=1,active(ilevel)%ngrid
           unew(active(ilevel)%igrid(i),ivar) = &
                & uold(active(ilevel)%igrid(i),ivar)
        end do
     end do
     if(pressure_fix)then
        do i=1,active(ilevel)%ngrid
           d=uold(active(ilevel)%igrid(i),1)
           u=0.0; v=0.0; w=0.0
           if(ndim>0)u=uold(active(ilevel)%igrid(i),2)/d
           if(ndim>1)v=uold(active(ilevel)%igrid(i),3)/d
           if(ndim>2)w=uold(active(ilevel)%igrid(i),4)/d
           e=uold(active(ilevel)%igrid(i),ndim+2)-0.5*d*(u**2+v**2+w**2)
           enew(active(ilevel)%igrid(i))=e
        end do
        do i=1,active(ilevel)%ngrid
  ...
  do ind=1,twotondim
     ...
     if(pressure_fix)then
        do i=1,reception(icpu,ilevel)%ngrid
           enew(reception(icpu,ilevel)%igrid(i))=0.0
        end do
        do i=1,reception(icpu,ilevel)%ngrid
...
subroutine set_uold(ilevel)
  ...
  real(dp)::d,u,v,w,e_kin,e_cons,e_prim,e_trunc,div,dx,fact
  ...
  do ind=1,twotondim
     ...
     do i=1,active(ilevel)%ngrid
        uold(active(ilevel)%igrid(i),ndim+3) = max(uold(active(ilevel)%igrid(i),ndim+3),0.0_dp)
        uold(active(ilevel)%igrid(i),ndim+4) = max(uold(active(ilevel)%igrid(i),ndim+4),0.0_dp)
     end do
     ...
     if(pressure_fix)then
        ...
        do i=1,active(ilevel)%ngrid
           ...
           e_cons=uold(active(ilevel)%igrid(i),ndim+2)-e_kin
           e_prim=enew(active(ilevel)%igrid(i))*(1.0d0+fact* &
                & divu(active(ilevel)%igrid(i)))! Note: here divu=-div.u*dt
           div=abs(divu(active(ilevel)%igrid(i)))*dx/dtnew(ilevel)
           e_trunc=0.5*d*max(div,3.0*hexp*dx)**2
           if(e_cons<e_trunc)then
              uold(active(ilevel)%igrid(i),ndim+2)=e_prim+e_kin
           end if
        end do
     end if
     do i=1,active(ilevel)%ngrid
...
subroutine godfine1(ind_grid,ncache,ilevel)
  ...
  real(dp),dimension(1:nvector,if1:if2,jf1:jf2,kf1:kf2,1:nvar,ndim ),save::flux
     ...
     do i2=i2min,i2max
        ...
        if(poisson)then
           do ivar=1,ndim
              ...
              do i=1,ncache
                 if(exist_nbor(i))then
                    gloc(i,i3,j3,k3,ivar)=f(ind_cell(i),ivar)
                 else
                    ibuffer=ibuffer+1
                    gloc(i,i3,j3,k3,ivar)=g2(ibuffer,ind_son,ivar)
                 end if
              end do
           end do
        end if
  ...
  do idim=1,ndim
     ...
     do i2=i2min,i2max
        ...
        do ivar=1,nvar
           if(ivar==ndim+3.or.ivar==ndim+4)then
              do i=1,ncache
                 vel = uold(ind_cell(i),idim+1)/uold(ind_cell(i),1)
                 unew(ind_cell(i),ivar)=unew(ind_cell(i),ivar)+ &
                      & vel*(flux(i,i3   ,j3   ,k3   ,ivar,idim) &
        ...
        ! Update non conservative variables
        if(pressure_fix)then
     ...
     do ivar=1,nvar
        if(ivar==ndim+3.or.ivar==ndim+4)then
           ...
           do i3=i3min,i3max-i0
              do i=1,ncache
                 if(.not.exist_nbor(i))then
                    vel=uold(ind_buffer(i),idim+1)/uold(ind_buffer(i),1)
                    unew(ind_buffer(i),ivar)=unew(ind_buffer(i),ivar) &
                         & -vel*flux(i,i3,j3,k3,ivar,idim)/dble(twotondim)
                 end if
              end do
           end do
           end do
           ...
           do i=1,ncache
              if(.not.exist_nbor(i))then
                 enew(ind_buffer(i))=enew(ind_buffer(i)) &
                      & -tmp (i,i3,j3,k3,2   ,idim)/dble(twotondim)
              end if
           end do
        end do
        end do
           ...
           do i3=i3min+i0,i3max
              do i=1,ncache
                 if(.not.exist_nbor(i))then
                    vel=uold(ind_buffer(i),idim+1)/uold(ind_buffer(i),1)
                    unew(ind_buffer(i),ivar)=unew(ind_buffer(i),ivar) &
                         & +vel*flux(i,i3+i0,j3+j0,k3+k0,ivar,idim)/dble(twotondim)
                 end if
              end do
           end do
           end do
     ...
     if(pressure_fix)then
        ...
        do i3=i3min+i0,i3max
           do i=1,ncache
              if(.not.exist_nbor(i))then
                 enew(ind_buffer(i))=enew(ind_buffer(i)) &
                      & +tmp (i,i3+i0,j3+j0,k3+k0,2   ,idim)/dble(twotondim)
              end if
           end do
        end do
        end do
