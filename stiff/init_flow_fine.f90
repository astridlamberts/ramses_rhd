...
subroutine init_flow_fine(ilevel)
  ...
  real(dp)::dx,rr,vx,vy,vz,ek,ei,scale_T2,xx1,xx2,xx3,dx_loc
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        do idim=1,ndim
           do i=1,ngrid
              xx(i,idim)=(xx(i,idim)-skip_loc(idim))*boxlen/dble(nx_loc)
           end do
        end do
        ...
        do idim=1,ndim
           do i=1,ngrid
              uu(i,idim+1)=uu(i,idim+1)*dble(nx_loc)/boxlen
           end do
        end do
        do i=1,ngrid
           uu(i,ndim+2)=uu(i,ndim+2)*(dble(nx_loc)/boxlen)**2
           uu(i,ndim+4)=uu(i,ndim+4)*(dble(nx_loc)/boxlen)**2
        end do
        ! Scatter variables
        do ivar=1,nvar
           do i=1,ngrid
     ...
     if(region_type(k) .eq. 'square')then
        ...
        do i=1,nn
           ...
           if(r<1.0)then
              ...
              q(i,ndim+3)=1.0/(gamma_region(k)-1.0)
              q(i,ndim+4)=gamma_region(k)*pinf_region(k)/(gamma_region(k)-1.0)
           end if
        end do
     end if
