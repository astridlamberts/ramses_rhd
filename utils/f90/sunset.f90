program sunset
  ...
  ! - a particle file containing stars, with position, velocity, age and metallicity
  ...
  real(KIND=8)::mtot,ddx,ddy,ddz,dex,dey,dez,time,time_tot,time_simu,weight
  real(KIND=8)::xmin=0,xmax=1,ymin=0,ymax=1,zmin=0,zmax=1,col,dust_opacity
  real(KIND=8)::max_column,intssp_dust
  ...
  character(LEN=128)::dustfile='/home/cont003/teyssier/dust/kext_albedo_WD_MW_3.1_60.txt'
  character(LEN=128)::sspfile='/home/cont003/teyssier/stardust99/Patrik-imfKroupa-Zmulti-subsampled.dat'
!  character(LEN=128)::sspfile='/home/cont003/teyssier/bc03/bruzual_charlot_2003.dat'
  character(LEN=128)::filterfile='/home/cont003/teyssier/pegase/filters.dat'
  ...
  do k=1,ncpu_read
     ...
     do i=1,npart2
        ...
        if(ok_part)then
           ...
           time=(time_simu-time)/(h0*1d5/3.08d24)/(365.*24.*3600.)        
!!$           if(time<1d9)then
!!$              write(*,*)age(i),tau_frw(iii-1),tau_frw(iii)
!!$              write(*,*)time/1d9,id(i)
!!$           endif

           if(time<15d6)m(i)=0d0

           itime=1
           do while(tssp(itime)<time.and.itime<ntime)
              itime=itime+1
           end do

           ihist=int(time/20d9*dble(nhist))
  ...
  do ihist=0,nhist
     write(10,*)dble(ihist)/dble(nhist)*20d9,time_hist(ihist)
  end do
  close(10)
  ...
  if(TRIM(filetype).eq.'asc')then
     open(unit=10,file=nomfich,form='formatted')
     allocate(toto(nx,ny))
     toto=log10(map(0:nx-1,0:ny-1)+1d-20)
     write(format_grille,*) nx
     do i=0,ny-1
        write(10,fmt='('//trim(format_grille)//'(2x,E15.8))' ) , toto(:,i)
     end do
     close(10)
  endif
