...
for i=10,last do begin   ;next 99 outputs
    ;window, 
    read_bin_ath, 'no_HD_1e9.00'+strtrim(i,2)+'.bin',d,px,py,pz,e=e,d_n=d_n,/ion;,par =[2,1,1]
    h=d(*,0,0)
    g=d_n(*,0,0)
    y=(g/h)*1.0d0
    shock[i]=max(where(y lt 0.9))
...
;window,1
psopen,'R_front_1e9.ps',/color;array with the physical time in s at each timestep
;set_plot,'ps'
