pro turb_to_planionturb_bc, fname, outname, lbox, dbar, temp, dtout, mhd=mhd, $
                     nprad=nprad, raddir=raddir, radint=radint, rs=rs, $
                     parallel=parallel, seed=seed, swap=swap, ib_x1=ib_x1,$
         ib_x2=ib_x2,ib_x3=ib_x3,ob_x1=ob_x1, ob_x2=ob_x2,ob_x3=ob_x3,CFL=CFL

; converts an isothermal MHD or hydro run with mean density 1,
; sound speed = 1, and box running from 0 to 1 to a dimensional run
; with chosen box size, density, initial temperature, and embedded
; radiatiors. It can change boundary conditions
...
for n=0,nproc-1 do begin
    ...
    while dummy ne '<par_end>' do begin
        ...
        if strmid(dummy,0,2) eq 'Nx' then begin
            ...
            pos=strpos(dummy,'=')+1
            ioutstr=strmid(dummy,0,pos+1)+string(CFL)
    ...
    printf, fpout, ''
    printf, fpout, 'ENERGY'
; part changed to include the case without magentic field
    if keyword_set(mhd) then begin
       e=double(0.5*(px^2+py^2+pz^2)/d + 0.5*(bx^2+by^2+bz^2) + $
             d*kb*temp/(mu*(gamma-1))) 
           endif else begin 
       e=double(0.5*(px^2+py^2+pz^2)/d + d*kb*temp/(mu*(gamma-1)))
    endelse
    writeu, fpout, e
; added to remove mhd
    if keyword_set(mhd) then begin
  ...
  ; endif
    ...
    for m=0,nprad-1 do begin      
            writeu, fpout, long(raddir[m])
             ...
             print,'raddir radint',radint[m],raddir[m]
    endfor
    printf, fpout, ''
