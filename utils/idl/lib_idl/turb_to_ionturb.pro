pro turb_to_ionturb, fname, outname, lbox, dbar, temp, dtout, mhd=mhd, $
                     nrad=nrad, xrad=xrad, srad=srad, rs=rs, $
                     parallel=parallel, seed=seed, swap=swap
...
for n=0,nproc-1 do begin
    ...
    writeu, fpout, bz
    printf, fpout, ''
    printf, fpout, 'RADIATOR LIST'
    writeu, fpout, long(nrad)
    if nrad eq 1 then xrad=reform(xrad,1,3)
    for m=0,nrad-1 do begin
        for l=0,2 do begin
            writeu, fpout, double(xrad[m,l])
        endfor
        writeu, fpout, double(srad[m])
        writeu, fpout, 0L
        writeu, fpout, float([1,0,0,0,1,0,0,0,1])
    endfor
    printf, fpout, ''
    printf, fpout, 'RAN2 STATE'
    writeu, fpout, idum
    writeu, fpout, idum2
    writeu, fpout, iy
    writeu, fpout, iv
    printf, fpout, ''
    printf, fpout, 'NEUTRAL DENSITY'
    writeu, fpout, d
    printf, fpout, ''
