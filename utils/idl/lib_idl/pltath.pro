...
COMMON SHAREI,dn    ; neutral density for ionization
  ...
  if ((block le 0) and (not eof(1))) then begin
    ...
    if (block lt 20) then begin
      ...
      if (block eq 11) then begin
        readscalarblock,1,d
        nvar = nvar + 1
      end else if (block eq 19) then begin
        print,"Assuming scalar[0] is neutral density"
        readscalarblock,1,dn
        ionize = 1
...
if (string eq "SCALARS density float") then begin
  ...
  return,22
end else if (string eq "SCALARS scalar[0] float") then begin
  return,19
