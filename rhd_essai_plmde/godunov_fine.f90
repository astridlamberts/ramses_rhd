...
subroutine godfine1(ind_grid,ncache,ilevel)
  ...
  real(dp)::var1,var2,var3,var4,var5
  ...
  do i1=i1min,i1max
     ...
     do j=0,twondim
        do ivar=1,nvar
           do i=1,nbuffer
              u1(i,j,ivar)=uold(ibuffer_father(i,j),ivar)
              if (u1(i,j,1) <0) then
                 write(*,*),u1(i,j,1),i,j,uold(ibuffer_father(i,j),1),'avant interpol'
              endif
              if (u1(i,j,5) <0) then
                 write(*,*),u1(i,j,5),i,j,uold(ibuffer_father(i,j),5),'avant interpol'
              endif
     ...
     do j=0,twondim
        do ivar=1,nvar
           do i=1,nbuffer
              if (u1(i,j,1) <0) then
                 write(*,*),u1(i,j,1),i,j,'apres interpol'
              endif
              if (u2(i,j,1) <0) then
                 write(*,*),u2(i,j,1),i,j,'apres interpol'
              endif
     ...
     do i2=i2min,i2max
        ...
        do ivar=1,nvar
           do i=1,nexist
              ...
              if (uloc(i,i3,j3,k3,1) <0) then
                 write(*,*),uloc(i,i3,j3,k3,1),i,i3,j3,k3,'D uloc'
              endif
              if(uold(ind_cell(i),1) <0) then
                 write(*,*),uold(ind_cell(i),1),i,ind_cell(i),'D uold'
              endif
  ...
  do idim=1,ndim
     ...
     do i3=i3min,i3max+i0
        do ivar=1,nvar
           do i=1,ncache
              if (uloc(i,i3,j3,k3,1) <0) then
!                 write(*,*),uloc(i,i3,k3,k3,1),i,i3,j3,k3,'D'
              endif

              if (sqrt(uloc(i,i3,j3,k3,2)**2+uloc(i,i3,j3,k3,3)**2) >uloc(i,i3,j3,k3,5)) then
!                 write(*,*),sqrt(uloc(i,i3,j3,k3,2)**2+uloc(i,i3,j3,k3,3)**2),i,i3,j3,k3,'M>E'
              endif

              if (uloc(i,i3,j3,k3,5) <0) then
 !                write(*,*),uloc(i,i3,k3,k3,5),i,i3,j3,k3,'E'
              endif
  ...
  do idim=1,ndim
     ...
     do i3=i3min,i3max+i0
        do ivar=1,nvar
           do i=1,ncache
              if (uloc(i,i3,j3,k3,1) <0) then
!                 write(*,*),uloc(i,i3,k3,k3,1),i,i3,j3,k3,'D apres'
              endif

              if (sqrt(uloc(i,i3,j3,k3,2)**2+uloc(i,i3,j3,k3,3)**2) >uloc(i,i3,j3,k3,5)) then
 !                write(*,*),sqrt(uloc(i,i3,j3,k3,2)**2+uloc(i,i3,j3,k3,3)**2),i,i3,j3,k3,'M>E apres'
              endif

              if (uloc(i,i3,j3,k3,5) <0) then
  !               write(*,*),uloc(i,i3,k3,k3,5),i,i3,j3,k3,'E apres'
              endif
  ...
  !--------------------------------------
   do idim=1,ndim
     ...
     do i2=i2min,i2max
        ...
        do i=1,ncache
             var1=unew(ind_cell(i),1)
              var2=unew(ind_cell(i),2)
              var3=unew(ind_cell(i),3)
              var4=unew(ind_cell(i),4)
              var5=unew(ind_cell(i),5)
          ...
          enddo
          if ((nstep .gt.395 ) .and. (nstep .lt. 395) .and. (idim .eq. 1) .and. & 
               (ind_cell(i) .eq. 5480) .and. (i3 .eq. 1) .and. (j3 .eq.1) .and. (k3 .eq. 1) &
               .and. (i .eq. 161))  then
        !389-392     
             !395 397
             if (var1 <0) then
                !              write(*,*),var1,'D avant update',ind_cell(i)
                
             endif

             if (sqrt(var2**2+var3**2)>var5) then
  !               write(*,*),sqrt(var2**2+var3**3),'M>E avant update',ind_cell(i)

             endif

              if (var5 <0) then
 !                write(*,*),var5,'E avant update',ind_cell(i)
  !               write(*,*),flux(i,i3   ,j3   ,k3   ,5,idim), flux(i,i3+i0,j3+j0,k3+k0,5,idim),idim
              endif

              


              if (unew(ind_cell(i),1) <0) then
  !               write(*,*),unew(ind_cell(i),1),'D apres update',ind_cell(i),nstep
                 
  !              write(*,*),flux(i,i3   ,j3   ,k3   ,1,idim), flux(i,i3+i0,j3+j0,k3+k0,1,idim),idim
              endif

              if (sqrt(unew(ind_cell(i),2)**2+unew(ind_cell(i),3)**2) >unew(ind_cell(i),5)) then
  !               write(*,*),sqrt(unew(ind_cell(i),2)**2+unew(ind_cell(i),3)**2),'M>E apres update',ind_cell(i),i,nstep
  !               WRITE(*,*),i3,i0,j3,j0,k3,k0
  !               write(*,*),unew(ind_cell(i),2),'Mxnew',var2
              endif

!              if (unew(ind_cell(i),5) <0) then
   !              write(*,*),unew(ind_cell(i),5),'E apres update',i,ind_cell(i)
    !             write(*,*),i,i3,j3,k3,'apres',nstep
     !            write(*,*),flux(i,i3   ,j3   ,k3   ,5,idim), flux(i,i3+i0,j3+j0,k3+k0,5,idim),idim,var5
!             endif
             write(*,*),''
             write(*,*),'********************************'
              write(*,*),var1,var2,var3,var4,var5,ind_cell(i),i,i3,j3,k3,'var'

              write(*,*),unew(ind_cell(i),:),ind_cell(i),'unew'
              write(*,*),flux(i,i3   ,j3   ,k3   ,1,idim),&
                   flux(i,i3   ,j3   ,k3   ,2,idim),flux(i,i3   ,j3   ,k3   ,3,idim), &
                   flux(i,i3   ,j3   ,k3   ,4,idim),flux(i,i3   ,j3   ,k3   ,5,idim),&
                   ind_cell(i),'f gauche'
              WRITE(*,*),flux(i,i3+i0,j3+j0,k3+k0,1,idim),flux(i,i3+i0,j3+j0,k3+k0,2,idim),&
                   flux(i,i3+i0,j3+j0,k3+k0,3,idim),flux(i,i3+i0,j3+j0,k3+k0,4,idim),&
                   flux(i,i3+i0,j3+j0,k3+k0,5,idim),ind_cell(i),'f droit'
