     ...
     do j = jlo, jhi
        do i = ilo, ihi
           do l= 1,ngrid
              ...
              velsq=u*u+v*v+w*w ; lor=(one-velsq)**(-half) ; lorsq=lor*lor
              ...
              h     = one+entho*p/r   !enthalpy
              kappa = entho*one/r      !dh/dp
              chi   = -entho*p/r**two     !dh/dr

              f=lorsq*(-h+h*r*kappa+r*chi*velsq) !normalizing factor
              
              a11= u*f
              a12= h*r*lorsq*(r*kappa-one)
              a15= u*(-r*lorsq*kappa+r*lorsq*velsq*kappa+one)
...
subroutine trace2d(q,dq,qm,qp,dx,dy,dt,ngrid)
  ...
  smallp = smallr*smallc**two/gamma
     ...
     do j = jlo, jhi
        do i = ilo, ihi
           do l = 1, ngrid
              ...
              velsq=u**two+v**two+w**two ; lorsq=one/(one-velsq) ; lor=sqrt(lorsq)
              vtot=sqrt(u**two+v**two+w**two)
              ...
              a22= u*lorsq*(-h+r*chi+r*h*kappa)
              a25= (r*lorsq*kappa*h-r*h*kappa*lorsq*u**2-h-h*lorsq*(v**two+w**two)+lorsq*r*chi*(v**two+w**two))/(r*lorsq*h)
     ...
     do j = jlo, jhi
        do i = ilo, ihi
           do l = 1, ngrid
              ...
              a22= u*lorsq*(-h+r*chi+r*h*kappa)
              a25= (r*lorsq*kappa*h-r*h*kappa*lorsq*u**two-h-h*lorsq*(v**two+w**two)+lorsq*r*chi*(v**two+w**two))/(r*lorsq*h)
              ...
              b15= v*(-r*lorsq*kappa+r*lorsq*velsq*kappa+one)
              ...
              b33= v*lorsq*(-h+r*chi+r*h*kappa)
              b35= (r*lorsq*kappa*h-r*h*kappa*lorsq*v**two-h-h*lorsq*(u**two+w**two)+lorsq*r*chi*(u**two+w**two))/(r*lorsq*h)
              ...
              c11= w*f
              c14= h*r*lorsq*(r*kappa-one)
              c15= w*(-r*lorsq*kappa+r*lorsq*velsq*kappa+one)
              ...
              c45=(r*lorsq*kappa*h-r*h*kappa*lorsq*w**two-h-h*lorsq*(u**two+v**two)+lorsq*r*chi*(u**two+v**two))/(r*lorsq*h)
              
              c54=-r**two*h*lorsq*chi
              c55= lorsq*w*(-h+r*chi+h*r*kappa)
              

              ! Source terms (including transverse derivatives)
              sr0 = (-a11*drx-a12*dux                -a15*dpx)*dtdx/f + (-b11*dry        -b13*dvy        -b15*dpy)*dtdy/f + (-c11*drz                -c14*dvz-c15*dpz)*dtdz/f
              su0 = (        -a22*dux                -a25*dpx)*dtdx/f + (        -b22*duy-b23*dvy        -b25*dpy)*dtdy/f + (        -c22*duz        -c24*dvz-c25*dpz)*dtdz/f
              sv0 = (        -a32*dux-a33*dvx        -a35*dpx)*dtdx/f + (                -b33*dvy        -b35*dpy)*dtdy/f + (                -c33*dvz-c34*dvz-c35*dpz)*dtdz/f
              sw0 = (        -a42*dux        -a44*dwx-a45*dpx)*dtdx/f + (                -b43*dvy-b44*dwy-b45*dpy)*dtdy/f + (                        -c44*dwz-c45*dpz)*dtdz/f
              sp0 = (        -a52*dux                -a55*dpx)*dtdx/f + (                -b53*dvy        -b55*dpy)*dtdy/f + (                        -c54*dvz-c55*dpz)*dtdz/f
...
!  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2)::c  
  ...
  real(dp) :: two=2.d0,one=1d0,half=.5d0
...
!                write (*,*) 'Switch...ctoprim'
                ...
                lor=one/1.e-4
                entho=one+gamma/(gamma-one)*q(l,i,j,k,5)/q(l,i,j,k,1)

                q(l,i,j,k,2) = Mx/M*(lor**two-one)**half/lor
                q(l,i,j,k,3) = My/M*(lor**two-one)**half/lor
                q(l,i,j,k,4) = Mz/M*(lor**two-one)**half/lor

                uin(l,i,j,k,1)=q(l,i,j,k,1)*lor
                uin(l,i,j,k,2)=q(l,i,j,k,1)*lor**two*entho*q(l,i,j,k,2)
                uin(l,i,j,k,3)=q(l,i,j,k,1)*lor**two*entho*q(l,i,j,k,3)
                uin(l,i,j,k,4)=q(l,i,j,k,1)*lor**two*entho*q(l,i,j,k,4)
                uin(l,i,j,k,5)=q(l,i,j,k,1)*lor**two*entho - q(l,i,j,k,5)
             ...
             q(l,i,j,k,5)=(gamma-one)/gamma*Xsi
     ...
     !389 392
            ! if (mx*q(l,i,j,k,2) <0)then
           !     write(*,*),Mx,q(l,i,j,k,2),l,i,j,k
                !                 write(*,*),u,l,i,j,k
...
subroutine uslope(q,dq,dx,dt,ngrid)
  ...
  real(dp)::dx,dt
  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2,1:nvar)::q 
  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2,1:nvar+1,1:ndim)::dq
...
function f_Mignone(R,D,M,Eprim,gamma)
  ...
  u2=M**two/((R+D)**two-M**two) ; lor=(one+u2)**(half)
  Xsi=(R-u2/(lor+one)*D)/lor**two
  P=(gamma-one)/gamma*Xsi
  f_Mignone  = R-P-Eprim
  return
...
function f_prim_Mignone(R,D,M,Eprim,gamma)
  ...
  real(dp)::u2,lor,dpdR
  u2=M**two/((R+D)**two-M**two) ; lor=(1+u2)**(half)
  dpdR=(gamma-one)/gamma
  dpdR=dpdR*(one+M**two/(R+D)**two*(one-D*lor/(R+D)))
  f_prim_Mignone=one-dpdR
  return
