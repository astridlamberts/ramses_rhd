...
!	  fm=0.
	  do i3=1,np3
	    ...
	    do i2 = 1,np2
	       do i1 = 1,np1
		  sigma=sigma+f(i1,i2,i3)**2
!		  fm=max(abs(f(i1,i2,i3)),fm)
	       end do
	    end do
	  end do
	  close(11)
	  if(icomp.eq.0)then
	     write(*,*)'icomp, sigma, max=',
     &           icomp,real(sqrt(sigma/(np1*np2*np3))),fm
	  else
	     write(*,*)'icomp, sigma, max=',
     &           icomp,real(sqrt(sigma/(np1*np2*np3))/vfact/dxr),
     &           fm/dxr,' dx'
	  endif
	else
...
c  This function calculates the variance of displacement with a Hanning
c  filter at the grid Nyquist frequency.
c
	implicit none
