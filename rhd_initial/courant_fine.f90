...
!  real(dp),dimension(:,:),allocatable::q
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim        
        ...
        do i=1,nleaf
           call ctoprimbis(uu,1,q)
           mass_loc=mass_loc+q(1,1)*vol
           v=sqrt(q(1,2)**2+q(1,3)**2+q(1,4)**2)
           lor=(1d0-v**2)**(-1./2.)
           ekin_loc=ekin_loc+(lor*v**2*q(1,1)-q(1,1))*vol
           eint_loc=eint_loc+(q(1,5)/(gamma-1)+q(1,1))*vol
        end do
