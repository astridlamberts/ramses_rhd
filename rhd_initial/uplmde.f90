...
!             endif
              ...
              do n=6,nvar
                 a = q(l,i,j,k,n)     ! Cell centered values
                 v = q(l,i,j,k,3)
                 day = dq(l,i,j,k,n,2)    ! TVD slopes
                                  
                 ! Top state
                 spzero=(v    )*dtdy
                 if(v>zero)spzero=-project_out
                 azryright = half*(-one-spzero )*day
                 qp(l,i,j,k,n,2) = a + azryright 
                 
                 ! Bottom state
                 spzero=(v    )*dtdy
                 if(v<=zero)spzero=+project_out
                 azryleft = half*(+one-spzero )*day
                 qm(l,i,j,k,n,2) = a + azryleft 
        ...
        do i = ilo, ihi
           do l = 1, ngrid
              ...
              do n=6,nvar
                 ...
                 spzero = (w    )*dtdy
                 if(w>zero)spzero=-project_out
                 azrright = half*(-one-spzero )*daz
                 qp(l,i,j,k,n,3) = a + azrright
                 
                 ! Back state
                 spzero = (w    )*dtdy
                 if(w<=zero)spzero=+project_out
                 azrleft = half*(+one-spzero )*daz
                 qm(l,i,j,k,n,3) = a + azrleft
