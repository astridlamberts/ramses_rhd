subroutine write_screen
           ...
           do ind=1,twotondim
              ...
              do i=1,ncache
                 if(son(ind_cell(i))==0)then
                    ...
                    if (M>E) then
                       write (*,*) 'M>E  write screen',D,M,E
                    endif
                    if ((E**2<M**2+D**2).or.(E<0)) then
                       
                       write (*,*) 'Switch...write screen'
                       
                       qq(icell,1) = smallr
                       qq(icell,5)   = smallr
        
                       lor=1./1.e-4
                       entho=1.d0+gamma/(gamma-1.d0)*qq(icell,5)/qq(icell,1)
        
                       qq(icell,2) = Mx/M*(lor**2-1.d0)**0.5/lor
                       qq(icell,3) = My/M*(lor**2-1.d0)**0.5/lor
                       qq(icell,4) = Mz/M*(lor**2-1.d0)**0.5/lor
                       
                       uold(ind_cell(i),1)=qq(icell,1)*lor
                       uold(ind_cell(i),2)=qq(icell,1)*lor**2*entho*qq(icell,2)
                       uold(ind_cell(i),3)=qq(icell,1)*lor**2*entho*qq(icell,3)
                       uold(ind_cell(i),4)=qq(icell,1)*lor**2*entho*qq(icell,4)
                       uold(ind_cell(i),5)=qq(icell,1)*lor**2*entho- qq(icell,5)
  ...
  if(myid==1)then
     ...
     do i=1,ncell
        ...
        !!!
        if ((uu(i)**2+vv(i)**2) .gt. 1.0d0) then 
           write(*,*)'argg'
           endif
     end do
     do i=1,ncell
