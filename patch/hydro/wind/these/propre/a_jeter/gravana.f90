  ...
  if(gravity_type==3)then 
    ...
    write(*,*),x01,y01,z01
     do i=1,ncell
        ...
        if(ndim>0)f(i,1)=-Gstar*m1**rxs/rrstar**3
        if(ndim>1)f(i,2)=-Gstar*m1*rys/rrstar**3
        if(ndim>2)f(i,3)=-Gstar*m1*rzs/rrstar**3
        if(ndim>0)f(i,1)=f(i,1)-Gstar*m2*rxp/rrpuls**3
        if(ndim>1)f(i,2)=f(i,2)-Gstar*m2*ryp/rrpuls**3
        if(ndim>2)f(i,3)=f(i,3)-Gstar*m2*rzp/rrpuls**3
