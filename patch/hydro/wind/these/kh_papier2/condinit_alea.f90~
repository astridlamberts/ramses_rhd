...
subroutine condinit(x,u,dx,nn)
  ...
  real(dp)::y0,p0,v0,v1,alpha,x0
  integer::ivar
  real(dp),dimension(1:nvector,1:nvar),save::q   ! Primitive variables
  real(dp)::rho1,rho2,v2,kx,ky,lambday,lambdaz,kz
  ...
  do i=1,nn
     if(x(i,2) < y0)then
        q(i,id)=1+alpha
        q(i,iu)=v1
        if(ndim>1)q(i,iu)=v1!v0*cos(ky*(x(i,2)-lambday/2.))*exp(+ky*(x(i,1)-x0))  
        if(ndim>1)q(i,iv)=v0*cos(ky*(x(i,1)-lambday/2.))*exp(+ky*(x(i,2)-y0))  
