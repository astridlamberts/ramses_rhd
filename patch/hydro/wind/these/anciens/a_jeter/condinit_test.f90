...
!               call reset_star(xc1,yc1,zc1,r01,dx,u_mask,i,ind_cell(i),ilevel,f1,vinf1,rstar1,rho1,v1,rc1)!#endif
               call reset_star(xc1,yc1,zc1,r01,dx,u_mask,i,ind_cell(i),ilevel,f1,vinf1,rstar1,rho1,v1,rc1,px(ind_cell(i)),py(ind_cell(i)),pz(ind_cell(i)))
           ...
           if (type2=='star')then
              ...
              call reset_star(xc2,yc2,zc2,r02,dx,u_mask,i,ind_cell(i),ilevel,f2,vinf2,rstar2,rho2,v2,rc2,px(ind_cell(i)),py(ind_cell(i)),pz(ind_cell(i)))
              px(ind_cell(i))=uold(ind_cell(i),2)
              py(ind_cell(i))=uold(ind_cell(i),3)
#if NDIM==3
              pz(ind_cell(i))=uold(ind_cell(i),4)
...
subroutine reset_star(xc,yc,zc,r0,delta_x,u_mask,i,ind_cell,ilevel,fs,vinf,rstar,rho,v,rc,pex,pey,pez)
...
!The routine modifies the global variable uold.
 ...
 ! the force works so there is an extra energy density related to the speed of the flow
      ! in the star v// force, no need for a real scalar product
...
!pressure
!  write(*,*),uold(ind_cell,ndim+2),c1,c2
...
real(kind=8)::rc
real(kind=8)::a=1.0d0
f3d=(vinf*v_a*rho_a*rstar*a)/(rc**4)
