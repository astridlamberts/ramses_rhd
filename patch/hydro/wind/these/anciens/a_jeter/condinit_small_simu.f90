...
subroutine condinit(x,u,dx,nn)
  ...
  real(dp)::r,ax,ay
!  real(dp),allocatable,dimension(:,:)::data_test
!  real(dp),dimension(4096,4096,4)::tab
!  real(dp),dimension(4096,4096)::tabd,tabvx,tabvy,tabp

  real(dp),allocatable,dimension(:,:,:)::tab_small_simu !array containing data of initial small size simulation
  real(dp),allocatable,dimension(:,:)::tabd,tabvx,tabvy,tabp,tabvz
  integer::size_small_simu=4096

! essai pour lire le fichier binaire (faudra faire un allocatable)

  allocate(tab_small_simu(size_small_simu,size_small_simu,ndim+2))
  allocate(tabd(size_small_simu,size_small_simu))
  allocate(tabvx(size_small_simu,size_small_simu))
  allocate(tabvy(size_small_simu,size_small_simu))
  allocate(tabp(size_small_simu,size_small_simu))
#if NDIM==3  
  allocate(tabvz(size_small_simu,size_small_simu))
#endif

  if ((allocated(tabd).neqv..true.).or.(allocated(tabvx).neqv..true.))then
      write(*,*),'problem to allocate arrays for the small simulation data'
  endif
  if((allocated(tabvy).neqv..true.).or.(allocated(tabp).neqv..true.)) then
      write(*,*),'problem to allocate arrays for the small simulation data'
  endif

#if NDIM==3
  if (allocated(tabvz).neqv..true.) then
     write(*,*),'problem to allocate vz for small simulation data'
  endif
#endif

   open(5, file="data.dat",form="unformatted")
   read(5),tab_small_simu
   close(5)
   tabd =tab_small_simu(:,:,1)
   tabvx=tab_small_simu(:,:,2)
   tabvy=tab_small_simu(:,:,3)
   tabp =tab_small_simu(:,:,ndim+2)
#if NDIM==3
   tabvz=tab(:,:,4)
#endif

     write(*,*),tabd(2000,2000),tabvx(2000,2000),tabvy(2000,2000),tabp(2000,2000)
 ...
 !positions of the second body
  ...
  if (rotation .eqv. .false.)then
     ...
     if (nstep == 1) then
        ...
        dt=dtnew(ilevel)/(nlevelmax-levelmin+1)
        vx02=vx02+ax02*dt/2.0d0
        vy02=vy02+ay02*dt/2.0d0
    endif   

  !The leapfrog routine is called at all the levels. So if there are 3 levels of
!  refinement the pulsar goes to fast by a factor of 3. We can either make a loop
! to pass only at the coarse level or divide the timestep according to the number of levels
! We divide the timesteps by the number of levels   nlevelmax-ilevel+1.

    dt=dtnew(ilevel)/(nlevelmax-levelmin+1)
    call leapfrog(x02,y02,vx02,vy02,dt,ilevel)
...
! a regrouper avec le reste du 3D?
         ...
         if (rc1 .lt. 1.2d0*r01) then
            ...
            if (type1=='star')then 
               ...
               call reset_star(xc1,yc1,zc1,r01,dx,u_mask,i,ind_cell(i),ilevel,f1,vinf1,a,rstar1,rho1,v1,rc1)!,px(ind_cell(i)),py(ind_cell(i)),pz(ind_cell(i)))
...
!#endif
           ...
           if (type2=='star')then
              ...
              call reset_star(xc2,yc2,zc2,r02,dx,u_mask,i,ind_cell(i),ilevel,f2,vinf2,a,rstar2,rho2,v2,rc2,pex,pey,pez)
              px(ind_cell(i))=uold(ind_cell(i),2)
              py(ind_cell(i))=uold(ind_cell(i),3)
#if NDIM==3
              pz(ind_cell(i))=uold(ind_cell(i),4)
...
subroutine reset_star(xc,yc,zc,r0,delta_x,u_mask,i,ind_cell,ilevel,fs,vinf,a,rstar,rho,v,rc)!,pex,pey,pez)  
...
!The routine modifies the global variable uold.
 ...
 ! the force works so there is an extra energy density related to the speed of the flow
      outm = uold(ind_cell,4)+fs*f(vinf,a,rstar,rho,v,rc)*vflu*dtnew(ilevel)
 ...
 ! the force works so there is an extra energy density related to the speed of the flow
      outm = uold(ind_cell,5)+fs*f(vinf,a,rstar,rho,v,rc)*vflu*dtnew(ilevel)
 ...
 ! the wind is accelerated by the force
      ...
      outm = uold(ind_cell,4)+yc/rc*dtnew(ilevel)*fs*f(vinf,a,rstar,rho,v,rc)          
      uold(ind_cell,4)=frac(xc,yc,zc,r0,delta_x)*u_mask(i,4)+(1.d0-frac(xc,yc,zc,r0,delta_x))*outm
...
#if NDIM==2
 fx1=  f1*f(vinf1,a,rstar1,rho1,v1,rc1)*xc1/rc1
 fy1=  f1*f(vinf1,a,rstar1,rho1,v1,rc1)*yc1/rc1
 fz1=  0
 fx2=  f2*f(vinf2,a,rstar2,rho2,v2,rc2)*xc2/rc2
 fy2=  f2*f(vinf2,a,rstar2,rho2,v2,rc2)*yc2/rc2
 fz2=  0

#endif
#if NDIM==3
 fx1=  f1*f3d(vinf1,a,rstar1,rho1,v1,rc1)*xc1/rc1
 fy1=  f1*f3d(vinf1,a,rstar1,rho1,v1,rc1)*yc1/rc1
 fz1=  f1*f3d(vinf1,a,rstar1,rho1,v1,rc1)*zc1/rc1
 fx2=  f2*f3d(vinf2,a,rstar2,rho2,v2,rc2)*xc2/rc2
 fy2=  f2*f3d(vinf2,a,rstar2,rho2,v2,rc2)*yc2/rc2
 fz2=  f2*f3d(vinf2,a,rstar2,rho2,v2,rc2)*zc2/rc2
#endif
...
!  c1 = (fx1*uold(ind_cell,2)+fy1*uold(ind_cell,3))*dtnew(ilevel)/uold(ind_cell,1)
!  c2 = (fx2*uold(ind_cell,2)+fy2*uold(ind_cell,3))*dtnew(ilevel)/uold(ind_cell,1)
...
!write(*,*),uold(ind_cell,4),rc1,uold(ind_cell,2),uold(ind_cell,1),c1,c2
...
!#if NDIM==3
!  c1 = zc1/rc1*f1*f(vinf1,a,rstar1,rho1,v1,rc1)*dtnew(ilevel) 
!  c2 = zc2/rc2*f2*f(vinf2,a,rstar2,rho2,v2,rc2)*dtnew(ilevel)                         
!  uold(ind_cell,4) = uold(ind_cell,4) +c1 +c2
!#endif
...
function f3d(vinf,a,rstar,rho_a,v_a,rc)
implicit none
real(kind=8)::f3d
real(kind=8)::vinf
real(kind=8)::a
...
f3d=(a**2**vinf*v_a*rho_a*rstar)/(rc**4)
