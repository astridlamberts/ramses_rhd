...
subroutine condinit(x,u,dx,nn)
  ...
  real(dp):: R_etoile, beta,rho_amb,P_amb,P0,rho0
  integer::  ivar
...
!  write(*,*),r0
  z0   = 0.5*boxlen
  R_etoile = 0.0539 !(if alpha=0.69)
  beta = 1.0
  !rho_amb = 1.09 *10**(-17)* eta*alpha**(-2)*mu**(-1)  !1.75*10**(-18)
  !cs_amb = 21*10**5*nu
  !P_amb = cs_amb**2*rho_amb/gamma
...
!  r0   = 0.5*boxlen*f0! Qui est f0?
!  rcut = 0.5*boxlen*fcut ! limite entre les 2 milieux
...
q(:,ip) = P_amb
  ...
  DO i=1,nn ! all the cells
     ...
     rc3=sqrt(xx**2+yy**2+zz*2)
       ...
       IF (rc3 .lt. 2*r0) THEN
        q(i,id) = rho0*(r0/rc)**2
        q(i,iu) = v_inf*(xx/rc3)*(1-R_etoile/rc3)**beta
        q(i,iv) = v_inf*(yy/rc3)*(1-R_etoile/rc3)**beta
        q(i,iw) = v_inf*(zz/rc3)*(1-R_etoile/rc3)**beta
        ...
        IF (rc .lt. 2*r0) THEN
           ...
           q(i,iu) = v_inf*(xx/rc)*max((1-R_etoile/rc)**beta,0.d0)
           q(i,iv) = v_inf*(yy/rc)*max((1-R_etoile/rc)**beta,0.d0)
