...
!!!!!debut ajout
!! pour reperer les positions, on est pas deja dans une tranche???????
...
! boucle pour le masque
           DO i=1,nn ! all the cells
              xc=xx(i,1)-x0 ! position cell/center
              yc=xx(i,2)-y0
              rc=sqrt(xc**2+yc**2)! 2d distance /center
#if NDIM == 3
              zc=xx(i,3)-z0
              rc3=sqrt(xc**2+yc**2+zc*2)
#endif
           

#if NDIM==3
              IF (rc3 .lt. 2*r0) THEN
                 uold(i,id) = rho0*(r0/rc)**2
...
end do ! fermeture boucle tranches!!!!!!!!!!!!!!
