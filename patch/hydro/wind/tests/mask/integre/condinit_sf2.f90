...
! a voir, pour le milieu ambiant j'utilise la premiere etoile comme reference!!!!!!!!!
  ...
  P_0 = 361*nu2*eta2*alpha2**(-2)
  rho_0 = 540*eta2*alpha2**(-2)*nu2**(-1)
  v_w2=nu2
  v_inf2 = 1.36*v_w2
...
!           q(i,iv) =  fract(xx,yy)*integration2D_vy(xx,yy,v_inf1)+(1.d0-fract(xx,yy))*q(i,iv)
          ...
          ! q(i,ip) = min(P_0*(r01/rc)**(10/3),2.d0**(2.d0*gamma)*P_0)
!           q(i,ip) = fract(xx,yy)*integration2D_P(xx,yy,P0_1)+(1.d0-fract(xx,yy))*q(i,ip)
...
! trier les variables inutiles!!!!!!!!!!!!!!!!!
  ...
  real(dp)::fract
  real(dp)::P_01,P0_2,rho_01,rho_02,v_w1,v_w2
  real(dp):: eta1, nu1, alpha1
  real(dp):: eta2, nu2, alpha2
  ...
  do ind=1,twotondim
   ...
   ! Gather cell centre positions
    ...
    ! Call reset of the mask routine, changes uold in the mask
     call condinit2(xx,u_mask,dx_loc,ngrid,coef_x01,coef_y01,coef_z01,coef_r01,rho_01,P_01,v_w1)
     call condinit2(xx,u_mask,dx_loc,ngrid,coef_x02,coef_y02,coef_z02,coef_r02,rho_02,P_02,v_w2)
  ...
  DO i=1,nn ! all the cells
     ...
     IF (rc3 .lt. 2*r0) THEN
        ...
        IF (rc .lt. 2*r0) THEN
           ...
           q(i,ip) = integration2D_P(xx,yy,P0)
