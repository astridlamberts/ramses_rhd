...
subroutine condinit(x,u,dx,nn,coef_x01,coef_y01,coef_z01,coef_r01,coef_x02,coef_y02,coef_z02,coef_r02)
  ...
  real(dp):: R_star1,R_star2, beta,rho_amb,P_amb,P_0,rho_0
  integer::  ivar
  ...
  DO i=1,nn ! all the cells
     ...
     IF (rc3 .lt. 2*r01) THEN
      ...
      ENDIF
        ...
        !write(*,*)'on est a 2D'
        IF (rc .lt. 2*r01) THEN
          ! q(i,id) =  fract(xx,yy)*integration2D_rho(xx,yy)+(1.d0-fract(xx,yy))*q(i,id)
          ! q(i,iu) =  fract(xx,yy)*integration2D_vx(xx,yy)+(1.d0-fract(xx,yy))*q(i,iu)
          ! q(i,iv) =  fract(xx,yy)*integration2D_vy(xx,yy)+(1.d0-fract(xx,yy))*q(i,iv)

           q(i,id) = integration2D_rho(xx,yy)
           q(i,iu) = integration2D_vy(xx,yy)
           q(i,iv) = integration2D_vx(xx,yy)
           q(i,ip) = integration2D_P(xx,yy)           
          ...
          ! q(i,ip) = min(P_0*(r01/rc)**(10/3),2.d0**(2.d0*gamma)*P_0)
          ! q(i,ip) = fract(xx,yy)*integration2D_P(xx,yy)+(1.d0-fract(xx,yy))*q(i,ip)
        ENDIF
#endif
   ENDDO     
  ...
  DO i=1,nn ! all the cells
     ...
     IF (rc3 .lt. 2*r0) THEN
        ...
        IF (rc .lt. 2*r0) THEN
           ...
           q(i,iu) = integration2D_vx(xx,yy)
           q(i,iv) = integration2D_vy(xx,yy)
...
function integration2D_rho(xx,yy)
implicit none
...
! computing the integral in the middle of the slice, be careful with size of the box!
         integral =integral +delta_x/n*delta_y/n*density2D((xx+(i-5-0.5d0)*delta_x/n),(yy+(j-5-0.5d0)*delta_y/n)) 
    end do
end do  
...
function integration2D_P(xx,yy)
implicit none
...
do i=1,n
   do j=1,n
! computing the integral in the middle of the slice, be careful with size of the box!
         integral =integral +delta_x/n*delta_y/n*pressure2D((xx+(i-5-0.5d0)*delta_x/n),(yy+(j-5-0.5d0)*delta_y/n)) 
    end do
end do  
...
function pressure2D(x,y) ! function which returns the density for a given position
implicit none
...
function integration2D_vx(xx,yy)
implicit none
...
do i=1,n
   do j=1,n
! computing the integral in the middle of the slice, be careful with size of the box!
      integral =integral +delta_x/n*delta_y/n*velocity2D_x((xx+(i-5-0.5d0)*delta_x/n),(yy+(j-5-0.5d0)*delta_y/n))
...
function integration2D_vy(xx,yy)
implicit none
...
do i=1,n
   do j=1,n
! computing the integral in the middle of the slice, be careful with size of the box!
         integral =integral +delta_x/n*delta_y/n*velocity2D_y((xx+(i-5-0.5d0)*delta_x/n),(yy+(j-5-0.5d0)*delta_y/n))
...
function velocity2D_x(x,y) ! function which returns the velocity for a given position
implicit none
real(kind=8):: velocity2D_x
real(kind=8)::x,y
...
function velocity2D_y(x,y) ! function which returns the velocity for a given position
implicit none
real(kind=8):: velocity2D_y
real(kind=8)::x,y
real(kind=8)::R_star = 0.0539d0 ! avec alpha, nu ,eta
...
!a refaire!!!!!!!!!!!!faut trouver la bonne position par rapport au centre du masque, pour chaque tranche de la cellule
do i=1,n
   do j=1,n
      r=sqrt(x**2+y**2)
      if (r .lt. r0)then
