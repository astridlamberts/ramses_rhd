...
!  c2 = (fx2*uold(ind_cell,2)+fy2*uold(ind_cell,3))*dtnew(ilevel)/uold(ind_cell,1)
  ...
  c1 = (fx1*vx+fy1*vy)*dtnew(ilevel)
  c2 = (fx2*vx+fy2*vy)*dtnew(ilevel)
...
P_r0=P_a*(r0/a)**gamma
   ...
   do i=1,n
      do j=1,n
       ...
       !   int =int +(dx/n)**ndim*P_a*min((a/r)**(gamma),(a/(0.01d0*r0))**(gamma)) 
 rho_r=Mdot/(2*3.1415*vinf*r)
!write(*,*),rho_a,rho_r
!int=int +(dx/n)**ndim*P_a*(rho_a/rho_r)**(-gamma)
!min((a/r)**(gamma),(a/(0.01d0*r0))**(gamma)) 
    ...
    do k=1,n
       do i=1,n
          do j=1,n
            ...
            int =int+(dx/n)**ndim*P_a*(a/r)**(gamma)
    !   int =int +(dx/n)**ndim*P_a*min((a/r)**(gamma),(a/(0.01d0*r0))**(gamma)) 
