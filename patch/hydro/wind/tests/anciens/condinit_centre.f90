...
subroutine condinit(x,u,dx,nn)
  ...
  allocate (px(ncell))
  allocate (py(ncell))
!write(*,*),allocated(px),allocated(py)
...
! a regrouper avec le reste du 3D?
         ...
         if (rc1 .lt. 1.2d0*r01) then
            ...
            if (type1=='star')then
                call reset_star(xc1,yc1,zc1,r01,dx,u_mask,i,ind_cell(i),ilevel,f1,vinf1,a,rstar1,rho1,v1,rc1,px(ind_cell(i)),py(ind_cell(i)))
             ...
             f2=0  
             call  reset_amb(xc1,yc1,zc1,rc1,xc2,yc2,zc2,rc2,ind_cell(i),ilevel,px(ind_cell(i)),py(ind_cell(i)))
             ...
             if (type2=='star')then
                  call reset_star(xc2,yc2,zc2,r02,dx,u_mask,i,ind_cell(i),ilevel,f2,vinf2,a,rstar2,rho2,v2,rc2,px(ind_cell(i)),py(ind_cell(i)))
...
subroutine reset_star(xc,yc,zc,r0,delta_x,u_mask,i,ind_cell,ilevel,fs,vinf,a,rstar,rho,v,rc,pex,pey)  
...
subroutine reset_amb(xc1,yc1,zc1,rc1,xc2,yc2,zc2,rc2,ind_cell,ilevel,pex,pey)
 !(xc1,yc1,zc1,rc1,xc2,yc2,zc2,rc2,ind_cell,ilevel,pex,pey)
  ...
  real(dp)::v1,v2,rho1,rho2,vflu
  integer::ind_cell,ilevel,i
  real(dp)::c1,c2,fv1,fv2,pex,pey,fx,fy,vx,vy
...
! write(*,*),f(vinf1,a,rstar1,rho1,v1,rc1)*(xc1/rc1*uold(ind_cell,2)+yc1/rc1*uold(ind_cell,3)),rc1
!fx= f1*f(vinf1,a,rstar1,rho1,v1,rc1)*(xc1/rc1*0.5d0*(uold(ind_cell,2)+pex))!

!fy= f1*f(vinf1,a,rstar1,rho1,v1,rc1)*(yc1/rc1*0.5d0*(uold(ind_cell,3)+pey))

!if ((f(vinf1,a,rstar1,rho1,v1,rc1)*(sqrt(((uold(ind_cell,2)+pex)/2)**2+((uold(ind_cell,3)+pey)/2)**2))&
!    -(fx+fy))/(fx+fy).gt. 0.001)then
!write(*,*),(f(vinf1,a,rstar1,rho1,v1,rc1)*(sqrt(((uold(ind_cell,2)+pex)/2.0d0)**2+((uold(ind_cell,3)+pey)/2.0d0)**2))&
!    -(fx+fy))/(fx+fy),xc1,yc1,rc1,fx,fy,(uold(ind_cell,2)+pex)/2.0d0,(uold(ind_cell,3)+pey)/2.0d0,f(vinf1,a,rstar1,rho1,v1,rc1),ind_cell,'f-f,x,y,r,fx,fy,vx,vy,f,i'
...
!if (Ec .gt. uold(ind_cell,4))then
!write(*,*),Ec,uold(ind_cell,4),ind_cell
!endif
