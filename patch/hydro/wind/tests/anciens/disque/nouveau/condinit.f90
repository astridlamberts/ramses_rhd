...
!     omega=sqrt(G*M/(rc)**3+cs**2*pe/(r_0*rc))
     omega=sqrt(GM/(rmask1**2+r3d**2)**(3./2.))
     !undirect rotation
     v=omega*r3d 
     q(i,iu)=v*yy/rc
     q(i,iv)=-v*xx/rc
 ENDDO     
   ...
   call init_star(q,x,nn,vinf1,rstar1,Mdot1,Mach1,x01,y01,z01,rmask1)
...
subroutine init_star(q,x,nn,vinf,rstar,Mdot,Mach,x0,y0,z0,r0)
  use wind_parameters
  use amr_parameters
  use hydro_parameters
  implicit none
  ...
  real(dp)::vinf,rstar,Mdot,Mach,x0,y0,z0,r0,vwind,vrot
...
!#endif
  ...
  DO i=1,nn ! all the cells
     ...
     IF (r3d .lt. 1.2d0*r0) THEN
        H=H_0*(rc/r_0)**((3.-qu)/2.)
        q(i,id)=rho_0/100*(r_0/r0)**pe
        q(i,ip)=cs**2*q(i,id)
        ! has to be identical to 3D case
        v=r3D*sqrt(GM/(r3D**2+r0**2)**(3./2.))
     ...
     IF (rc .lt. 1.2d0*r0) THEN
        q(i,id)=rho_0*(r_0/r0)**pe
        q(i,ip)=cs**2*q(i,id)
        v=rc*sqrt(GM/(rc**2+r0**2)**(3./2.))
        q(i,iu)=v*yy/rc
        q(i,iv)=-v*xx/rc
    ENDIF
#endif
...
!values for r=a (mettre dans une fonction)
  v = vinf1*(1.0d0-rstar1)
 rho = Mdot1/(4.0d0*3.1415*a**2*v)


#if NDIM==2
 rho = Mdot1/(2.0d0*3.1415*a*v)
#endif
...
! get size of cell
     ...
     do i=1,ngrid
         ...
         if (rc1 .lt. 1.2*rmask1) then
             call reset_star(xc1,yc1,zc1,rmask1,dx,u_mask,i,ind_cell(i),ilevel,vinf1,rstar1,rho,v,rc1)
...
subroutine reset_star(xc,yc,zc,r0,delta_x,u_mask,i,ind_cell,ilevel,vinf,a,rstar,rho,v,rc)
...
!The routine modifies the global variable uold.
  ...
  integer :: i,ilevel,j
  real(dp)::xc,yc,zc,delta_x,rc,r0,vinf,rstar,rho,v,a
  integer ::ind_cell    

  do j=1,nvar
      uold(ind_cell,j)=frac(xc,yc,zc,r0,delta_x)*u_mask(i,j)+(1.d0-frac(xc,yc,zc,r0,delta_x))*uold(ind_cell,j)
  enddo
...
!#endif
  ...
  DO i=1,nn ! all the cells
     ...
     r3d=sqrt(xx**2+yy**2+zz**2)
     rc=sqrt(xx**2+yy**2)
     cs=cs_0*(r_0/rmask1)**(qu/2.)
     IF (r3d .lt. 1.2d0*rmask1) THEN
          H=H_0*(rc/r_0)**((3.-qu)/2.)
          q(i,id)=rho_0/100.*(r_0/rmask1)**pe
          q(i,ip)=cs**2*q(i,id)
          v=r3d*sqrt(GM/(r3d**2+rmask1**2)**(3./2.))
          vwind=int_vx(xx,yy,zz,vinf1,rstar1,beta1,dx)
          vrot=v*yy/rc
          q(i,iu) = vwind+vrot
          vwind=int_vy(xx,yy,zz,vinf1,rstar1,beta1,dx)
          vrot=-v*xx/rc
          q(i,iv) =  vwind+vrot
          q(i,iw) =  int_vz(xx,yy,zz,vinf1,rstar1,beta1,dx)
        ! Convert primitive to conservative variables
        ! density -> density
        u(i,id)=q(i,id)
        ! velocity -> momentum
        ...
        ENDIF
           ...
           cs=cs_0*(r_0/rmask1)**(qu/2.)
#if NDIM==2   
        IF (rc .lt. 1.2d0*rmask1) THEN
           q(i,id)=rho_0*(r_0/rmask1)**pe
           q(i,ip)=cs**2*q(i,id)
           v=rc*sqrt(GM/(rc**2+rmask1**2)**(3./2.))
           q(i,iu)=v*yy/rc
           q(i,iv)=-v*xx/rc
           ! Convert primitive to conservative variables
           ! density -> density
...
namelist/disk/r_0,rho_0,pe,qu
namelist/star/GM,x01,y01,z01,rstar1,rmask1,vinf1,Mdot1,beta1
