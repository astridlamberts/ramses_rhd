...
!  real(dp):: rho_amb,P_amb,v_a,P_a,rho_a
  ...
  real::H,cs,v,Mach,rho_b,v_b,omega,r3d
...
!     omega=sqrt(G*M/(rc)**3+cs**2*pe/(r_0*rc))
   !  omega=sqrt(G*M/(r0**2+rc**2)**(3./2.))
     omega=sqrt(G*M/(r0**2+r3d**2)**(3./2.))
     v=omega*r3d !ou rc!
     q(i,iu)=v*yy/rc
     q(i,iv)=-v*xx/rc
 ENDDO     

   call init_star(q,x,nn,vinf,rstar,Mdot,Mach,x0,y0,z0,r0)
...
subroutine init_star(q,x,nn,vinf1,rstar1,Mdot1,Mach1,x01,y01,z01,r01)
  ...
  real(dp)::pi=3.141459265
   integer:: i,j,id,iu,iv,iw,ip
  real::H,cs,Mach,v_b,rho_b,density,rmask,v,R3D
  real(dp)::vinf1,rstar1,Mdot1,Mach1,x01,y01,z01,r01
 ...
 rho_a = Mdot/(2.0d0*pi*a*v_a)
  ...
  DO i=1,nn ! all the cells
     ...
     IF (r3d .lt. 1.2d0*r0) THEN
        H=H_0*(rc/r_0)**((3.-qu)/2.)
!        q(i,id)=rho_0*(r_0/r0)!**pe*exp(-zz**2/2.d0/H**2)
        q(i,id)=rho_0*(r_0/r0)**pe!*exp(-zz**2/2.d0/H**2)

        q(i,ip)=cs**2*q(i,id)
!        write(*,*),q(i,id),cs,q(i,ip)
        v=r3D*sqrt(G*M/(r3D**2+r0**2)**(3./2.))
        q(i,iu)=v*yy/rc
        q(i,iv)=-v*xx/rc
...
#endif
         if (rc1 .lt. 1.2*r0) then
             call reset_star(xc1,yc1,zc1,r0,dx,u_mask,i,ind_cell(i),ilevel,vinf,rstar,rho,v,rc1)
...
subroutine reset_star(xc,yc,zc,r0,delta_x,u_mask,i,ind_cell,ilevel,vinf1,a1,rstar1,rho,v,rc)
...
!The routine modifies the global variable uold.
  ...
  integer :: i,ilevel,j
  real(dp)::xc,yc,zc,delta_x,rc,r0,vinf1,a1,rstar1,rho,v
  integer ::ind_cell    
...
!it returns the vector u and requires all the parameters of the star as input
  ...
  real(dp):: delta_x
  real(dp):: boxlen_0,rmask
  real(dp)::v_a,rho_a,P_a,Mach,v_b,rho_b,v,r3d
  ...
  DO i=1,nn ! all the cells
     ...
     IF (r3d .lt. 1.2d0*r0) THEN
        ...
        !q(i,iv) =0.0d0
        q(i,iw) =0.0d0
        !q(i,id) =int_rho(xx,yy,zz,Mdot,r0,dx,a,vinf,rstar,beta)
        !q(i,ip) =  int_P(xx,yy,zz,P_a,rho_a,Mdot,a,r0,vinf,rstar,beta,gamma,dx)
        ! Convert primitive to conservative variables
        ! density -> density
        u(i,id)=q(i,id)
        ! velocity -> momentum
