...
!position of the first body, the center of mass is the center of the box
  x01 = 0.5*boxlen-(Mstar2)*a1/(Mstar1+Mstar2)
  ...
  if (rotation .eqv. .false.)then
    ...
    if (exc .ne. 0) then
       if (nstep == 1) then
           ...
           y02   = y01
           vx02=0.0d0
           vy02=sqrt(Gstar*Mstar1*(2.0D0/(x02-x01)-1.0d0/a1))
        
           r=sqrt((x02-x01)**2+(y02-y01)**2)
           ax02=-Gstar*Mstar1/r**2*(x02-x01)/r
           ay02=-Gstar*Mstar1/r**2*(y02-y01)/r
...
! get size of cell
     ...
     do i=1,ngrid
         ...
         if (rc1 .lt. 1.2d0*r01) then
            ...
            if (type1=='star')then 
               !if the star has accelerated winds, we use a centered scheme to update the force
               pex=px(ind_cell(i))
               pey=py(ind_cell(i))
               pey=pz(ind_cell(i))
               call reset_star(xc1,yc1,zc1,r01,dx,u_mask,i,ind_cell(i),ilevel,f1,vinf1,a,rstar1,beta1,rc1,pew,pey,pez)
           ...
           !case where there is only 1 body
             f2=0  
           if (f1.eq.1)then
              ! if the body has accelerated winds, the ambient medium has to be updated
           ...
           if (type2=='star')then
              ...
              pey=py(ind_cell(i))
              pey=pz(ind_cell(i))
              call reset_star(xc2,yc2,zc2,r02,dx,u_mask,i,ind_cell(i),ilevel,f2,vinf2,a,rstar2,beta2,rc2,pex,pey,pez)
              px(ind_cell(i))=uold(ind_cell(i),2)
              py(ind_cell(i))=uold(ind_cell(i),3)
#if NDIM==3
              pz(ind_cell(i))=uold(ind_cell(i),4)
...
subroutine reset_star(xc,yc,zc,r0,delta_x,u_mask,i,ind_cell,ilevel,fs,vinf,a,rstar,beta,rc,pex,pey,pez)
...
!The routine modifies the global variable uold.
  ...
  real(dp):: outm
  integer :: i,ilevel,fs
  real(dp)::xc,yc,zc,r0,delta_x,rc,vinf,a,rstar,rho,v,f,beta
  integer ::ind_cell    
  real(dp)::pex,pey,pez ! values of the momentum along each axis for the former time step
  real(dp)::fx,fy,fz,c
  real(dp):: a1=1.0d0
  real(dp)::v,rho
...
!values at 'a'
   v = vinf*(1.0d0-rstar)**beta 
 rho = Mdot*a1**2/(4.0d0*3.1415*v)

#if NDIM==2
 rho1 = Mdot*a1/(2.0d0*3.1415*v)
#endif

! compute the force along each direction
#if NDIM==2
 fx=  fs*f(vinf,a,rstar,rho,v,rc)*xc/rc
 fy=  fs*f(vinf,a,rstar,rho,v,rc)*yc/rc
 fz=  0
#endif
#if NDIM==3
 fx=  fs*f3d(vinf,a,rstar,rho,v,rc)*xc/rc
 fy=  fs*f3d(vinf,a,rstar,rho,v,rc)*yc/rc
 fz=  fs*f3d(vinf,a,rstar,rho,v,rc)*zc/rc
#endif
...
! dot product between force and velocity because the force works
  c = (fx*vx+fy*vy*fz*vz)*dtnew(ilevel)
  
!density
  uold(ind_cell,1)=frac(xc,yc,zc,r0,delta_x)*u_mask(i,1)+(1.d0-frac(xc,yc,zc,r0,delta_x))*uold(ind_cell,1)
!pressure
  uold(ind_cell,ndim+2)=frac(xc,yc,zc,r0,delta_x)*u_mask(i,ndim+2)+&
                 1.0d0-fract(xc,yc,zc,r0,delta_x)*uold(ind_cell,ndim+2)+c
!velocity
  uold(ind_cell,2)=frac(xc,yc,zc,r0,delta_x)*u_mask(i,2)+&
            (1.0d0-frac(xc,yc,zc,r0,delta_x)*(uold(ind_cell,2)+(fx1+fx2)*dtnew(ilevel))
  uold(ind_cell,3)=frac(xc,yc,zc,r0,delta_x)*u_mask(i,3)+&
            (1.0d0-frac(xc,yc,zc,r0,delta_x)*(uold(ind_cell,3)+(fy1+fy2)*dtnew(ilevel))
#if NDIM==3
  uold(ind_cell,4)=frac(xc,yc,zc,r0,delta_x)*u_mask(i,4)+&
            (1.0d0-frac(xc,yc,zc,r0,delta_x)*(uold(ind_cell,4)+(fz1+fz2)*dtnew(ilevel))
#endif
