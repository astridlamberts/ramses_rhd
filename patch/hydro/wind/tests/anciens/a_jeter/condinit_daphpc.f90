...
subroutine condinit(x,u,dx,nn)
  ...
  real(dp)::inc,peri,noed,theta,dthetadt
  real(dp):: ax1,ay1,az1,ax2,ay2,az2,r1,r2,r
...
!  gmpuls=7.!48.0d0
  ...
  x01=xc+M2/(M1+M2)*a1*(1-exc)*(cos(noeud)*cos(peri+theta)-sin(noeud)*sin(peri+theta)*cos(inc))
  y01=yc+M2/(M1+M2)*a1*(1-exc)*(sin(noeud)*cos(peri+theta)+cos(noeud)*sin(peri+theta)*cos(inc))
  z01=zc+M2/(M1+M2)*a1*(1-exc)*sin(peri+theta)*sin(inc)

  x02=x1-a1*(1-exc)*(cos(noeud)*cos(peri+theta)-sin(noeud)*sin(peri+theta)*cos(inc))
  y02=y1-a1*(1-exc)*(sin(noeud)*cos(peri+theta)+cos(noeud)*sin(peri+theta)*cos(inc))
  z02=z1-a1*(1-exc)*sin(peri+theta)*sin(inc)
...
!  write(*,*),r,xini1,xini2,yini1,yini2,zini1,zini2
  ...
  r1=sqrt((x01-xc)**2.0d0+(y01-yc)**2.0d0+(z01-zc)**2.0d0)
  r2=sqrt((x02-xc)**2.0d0+(y02-yc)**2.0d0+(z02-zc)**2.0d0)
  write(*,*),r,r1,r2
 
  if (direction .eq.'d') then 
  ! direct rotation
     
     vx01=dthetadt*(-cos(noeud)*sin(peri+theta)-sin(noeud)*cos(peri+theta)*cos(inc))*r1
     vy01=dthetadt*(-sin(noeud)*sin(peri+theta)+cos(noeud)*cos(peri+theta)*cos(inc))*r1
     vz01=dthetadt*cos(peri+theta)*sin(inc)*r1
     
     vx02=-dthetadt*(-cos(noeud)*sin(peri+theta)-sin(noeud)*cos(peri+theta)*cos(inc))*r2
     vy02=-dthetadt*(-sin(noeud)*sin(peri+theta)+cos(noeud)*cos(peri+theta)*cos(inc))*r2
...
subroutine leapfrog_bis(x,y,vx,vy,det,ilevel)
use wind_params
implicit none
