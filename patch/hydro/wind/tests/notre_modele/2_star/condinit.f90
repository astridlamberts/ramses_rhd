...
!run parameters
  ...
  r0    = 0.1!0.078d0*boxlen_0/boxlen

!  x01   =0.5d0*boxlen - (6.0d0/32.0d0) * boxlen
  x01   = 0.4*boxlen!0.5d0*(boxlen-0.4d0*boxlen_0/boxlen)
 ! x01   = 0.5d0*(boxlen-0.5d0*boxlen_0/boxlen)
  y01   = 0.5d0*boxlen
  z01   = 0.5d0*boxlen
  
 ! x02   =0.5d0*boxlen + (6.0d0/32.0d0) * boxlen
  x02   = 0.6*boxlen!0.5d0*(boxlen+0.4d0*boxlen_0/boxlen)
 ! x02   = 0.5d0*(boxlen+0.5d0*boxlen_0/boxlen)
  y02   = 0.5d0*boxlen
  z02   = 0.5d0*boxlen

  R_star1 = 0.05!0.0539*boxlen_0/boxlen
  R_star2 = 0.05!0.0539*boxlen_0/boxlen
...
! Second star :
  ...
  DO i=1,nn ! all the cells
     ...
     rc=sqrt(xx**2+yy**2+zz**2)
     IF (rc .lt. 2.0d0*r0) THEN
           q(i,id) =  fract(xx,yy,zz,r0,delta_x)*integration_rho(xx,yy,zz,rho_02,r0,delta_x)+(1.d0-fract(xx,yy,zz,r0,delta_x))*q(i,id)
           q(i,iu) =  fract(xx,yy,zz,r0,delta_x)*integration_vx(xx,yy,zz,v_inf2,R_star2,delta_x)+(1.d0-fract(xx,yy,zz,r0,delta_x))*q(i,iu)
           q(i,iv) =  fract(xx,yy,zz,r0,delta_x)*integration_vy(xx,yy,zz,v_inf2,R_star2,delta_x)+(1.d0-fract(xx,yy,zz,r0,delta_x))*q(i,iv)
           q(i,iw) =  fract(xx,yy,zz,r0,delta_x)*integration_vy(xx,yy,zz,v_inf2,R_star2,delta_x)+(1.d0-fract(xx,yy,zz,r0,delta_x))*q(i,iw)
           q(i,ip) = fract(xx,yy,zz,r0,delta_x)*integration_P(xx,yy,zz,P_02,r0,delta_x)+(1.d0-fract(xx,yy,zz,r0,delta_x))*q(i,ip)
      ENDIF
#endif
...
!a ameliorer!!!!!!!!!
  ...
  r0    =0.1 !0.30.078*boxlen_0/boxlen   
 ! x01   =0.5d0*boxlen - (6.0d0/32.0d0) * boxlen
  x01   = 0.4*boxlen!*boxlen0.5d0*(boxlen-0.4d0*boxlen_0/boxlen)
  !x01   = 0.5d0*(boxlen-0.5d0*boxlen_0/boxlen)
  y01   = 0.5d0*boxlen
  z01   = 0.5d0*boxlen
...
!run parameters
  ...
  R_star = 0.05!0.054d0*boxlen_0/boxlen
...
! computing the integral in the middle of the slice, be careful with size of the box!
         integral =integral +delta_x/n*delta_x/n*density((xx+(i-5-0.5d0)*delta_x/n),(yy+(j-5-0.5d0)*delta_x/n),zz,rho_0,r0) 
      end do
   end do
#endif
   ...
   do i=1,n
      do j=1,n
         do k=1,n
! computing the integral in the middle of the slice, be careful with size of the box!
            integral =integral +(delta_x/n)**ndim*density((xx+(i-5-0.5d0)*delta_x/n),(yy+(j-5-0.5d0)*delta_x/n),(zz+(k-0.5d0-5.0d0)*delta_x/n),rho_0,r0) 
         end do
      end do
   enddo
#endif
...
r=sqrt(x**2+y**2+z**2)
velocity_x=v_inf*(x/r)*max((1.0d0-R_star/r)**beta,0.d0)
...
r=sqrt(x**2+y**2+z**2)
velocity_y=v_inf*(y/r)*max((1.0d0-R_star/r)**beta,0.d0)
...
r=sqrt(x**2+y**2+z**2)
velocity_z=v_inf*(z/r)*max((1.0d0-R_star/r)**beta,0.d0)
