...
!run parameters
 ...
 ! r0    = 0.054d0*boxlen_0/boxlen
  ...
  x02   =0.5d0+alpha/(boxlen*2.0d0)
!*boxlen + (26.0d0/64.0d0)*boxlen!!*boxlen_0/boxlen
  !x02  = 0.5d0*boxlen +(anint(2.0d0**levelmax/2.5)/(2.0d0**levelmax))
!  x02   = 0.75d0*boxlen!(boxlen+0.5d0*boxlen_0/boxlen)
  y02   = 0.5d0*boxlen
  z02   = 0.5d0*boxlen


  R_star1 = 0.0539*boxlen_0/boxlen
  R_star2 = 0.0539*boxlen_0/boxlen
  ...
  Mdot_2=1.0d0/(4.d0*3.14d0*rho_02*v_w2*r0**2)

  eta_w=(Mdot_1*v_w1)/(Mdot_2*v_w2)
  
!  write(*,*),'eta wind vaut',eta_w


 ! There is an initial domain filled with a densité rho_amb and sound speed cs_amb. Afterwards, we add a mask (for r <2r0) with a given density, pressure and speed. 
...
!     write(*,*),'3D_star1_ini'
    ...
    ! write(*,*),rc,x(i,3),z01,zz, 'rc 1,x(i,3),z01,zz'
     IF (rc .lt. 2.0d0*r0) THEN
        q(i,id) =  fract(xx,yy,zz,r0,delta_x)*integration_rho(xx,yy,zz,rho_01,r0,delta_x)&
            +(1.d0-fract(xx,yy,zz,r0,delta_x))*q(i,id)
        q(i,iu) =  fract(xx,yy,zz,r0,delta_x)*integration_vx(xx,yy,zz,v_inf1,R_star1,delta_x)&
            +(1.d0-fract(xx,yy,zz,r0,delta_x))*q(i,iu)
        q(i,iv) =  fract(xx,yy,zz,r0,delta_x)*integration_vy(xx,yy,zz,v_inf1,R_star1,delta_x)&
            +(1.d0-fract(xx,yy,zz,r0,delta_x))*q(i,iv)
        q(i,iw) =  fract(xx,yy,zz,r0,delta_x)*integration_vz(xx,yy,zz,v_inf1,R_star1,delta_x)&
            +(1.d0-fract(xx,yy,zz,r0,delta_x))*q(i,iw)
           q(i,ip) =  fract(xx,yy,zz,r0,delta_x)*integration_P(xx,yy,zz,P_01,r0,delta_x)&
            +(1.d0-fract(xx,yy,zz,r0,delta_x))*q(i,ip)
  ...
  DO i=1,nn ! all the cells
    ...
    ! write(*,*),'3D_star2_ini'
     ...
     IF (rc .lt. 2.0d0*r0) THEN
        ...
        q(i,id) =  fract(xx,yy,zz,r0,delta_x)*integration_rho(xx,yy,zz,rho_02,r0,delta_x)&
            +(1.d0-fract(xx,yy,zz,r0,delta_x))*q(i,id)
        q(i,iu) =  fract(xx,yy,zz,r0,delta_x)*integration_vx(xx,yy,zz,v_inf1,R_star2,delta_x)&
            +(1.d0-fract(xx,yy,zz,r0,delta_x))*q(i,iu)
        q(i,iv) =  fract(xx,yy,zz,r0,delta_x)*integration_vy(xx,yy,zz,v_inf1,R_star2,delta_x)&
            +(1.d0-fract(xx,yy,zz,r0,delta_x))*q(i,iv)
        q(i,iw) =  fract(xx,yy,zz,r0,delta_x)*integration_vz(xx,yy,zz,v_inf1,R_star2,delta_x)&
            +(1.d0-fract(xx,yy,zz,r0,delta_x))*q(i,iw)
           q(i,ip) =  fract(xx,yy,zz,r0,delta_x)*integration_P(xx,yy,zz,P_02,r0,delta_x)&
            +(1.d0-fract(xx,yy,zz,r0,delta_x))*q(i,ip)  
      ENDIF
#endif
...
#if NDIM==2   
        IF (rc .lt. 2.0d0*r0) THEN
         !  write(*,*),zz,'remplissage2'
           q(i,id) =  fract(xx,yy,zz,r0,delta_x)*integration_rho(xx,yy,zz,rho_02,r0,delta_x)&
               +(1.d0-fract(xx,yy,zz,r0,delta_x))*q(i,id)
           q(i,iu) =  fract(xx,yy,zz,r0,delta_x)*integration_vx(xx,yy,zz,v_inf2,R_star2,delta_x)&
               +(1.d0-fract(xx,yy,zz,r0,delta_x))*q(i,iu)
           q(i,iv) =  fract(xx,yy,zz,r0,delta_x)*integration_vy(xx,yy,zz,v_inf2,R_star2,delta_x)&
               +(1.d0-fract(xx,yy,zz,r0,delta_x))*q(i,iv)
           q(i,ip)  = fract(xx,yy,zz,r0,delta_x)*integration_P(xx,yy,zz,P_02,r0,delta_x)&
               +(1.d0-fract(xx,yy,zz,r0,delta_x))*q(i,ip)
...
#endif
#if NDIM>2
  u(1:nn,5)=u(1:nn,ndim+2)+0.5*q(1:nn,1)*q(1:nn,4)**2
#endif
  ! pressure -> total fluid energy
  u(1:nn,ndim+2)=u(1:nn,ndim+2)+q(1:nn,ndim+2)/(gamma-1.0d0)
  
!do i =1,nn
!   write(*,*),i,u(i,5),'i,Etot'
!end do
  ! passive scalars
  !do ivar=ndim+3,nvar
...
! trier les variables inutiles!!!!!!!!!!!!!!!!!
  ...
  real(dp)::x01,y01,z01,rc,xcen,ycen,zcen,r0,x02,y02,z02
  real(dp)::fract
...
!   x02  = 0.5d0*boxlen +(anint(2.0d0**levelmax/2.5)/(2.0d0**levelmax))
!  x02   =0.5d0*boxlen + (13.0d0/64.0d0)*boxlen
! 0.75d0*boxlen!*(boxlen+0.5d0*boxlen_0/boxlen)
  y02   = 0.5d0*boxlen
  z02   = 0.5d0*boxlen
         ...
         do i=1,ngrid
          ...
          !  write(*,*),'3Dreset mask'
            ...
            if (rc .lt. 2.0d0*r0) then
             !  write(*,*),zcen,rc,'z,r 1'
            !   write(*,*),ivar,'ivar'
!               write(*,*),uold(ind_cell(i),ivar),u_mask(i,ivar),ivar,'uold,umask,ivar'
               uold(ind_cell(i),ivar)=fract(xcen,ycen,zcen,r0,delta_x)*u_mask(i,ivar)&
                    +(1.d0-fract(xcen,ycen,zcen,r0,delta_x))*uold(ind_cell(i),ivar)
            endif
            ...
            rc=sqrt(xcen**2+ycen**2)
#if NDIM ==3
           ! write(*,*),'3D reset mask 2'
            zcen=xx(i,3)-z02
            rc=sqrt(xcen**2+ycen**2+zcen**2) 
#endif
            if (rc .lt. 2.0d0*r0) then
!               write(*,*),rc,r0,'rc,r0,star2'
              ! write(*,*),zcen,rc,'z,r 1'
              uold(ind_cell(i),ivar)=fract(xcen,ycen,zcen,r0,delta_x)*u_mask(i,ivar)&
                   +(1.d0-fract(xcen,ycen,zcen,r0,delta_x))*uold(ind_cell(i),ivar)
            endif
...
! computing the integral in the middle of the slice, be careful with size of the box!
         integral =integral +delta_x/n*delta_x/n*density((xx+(i-5.0d0-0.5d0)*delta_x/n)&
              ,(yy+(j-5.0d0-0.5d0)*delta_x/n),zz,rho_0,r0) 
       end do
   end do  
#endif

#if NDIM==3
   !write(*,*),rho_0,zz,'3D integr rho,rho_0,zz'
   do i=1,n
       do j=1,n
          do k=1,n
! computing the integral in the middle of the slice, be careful with size of the box!
             integral =integral +(delta_x/n)**ndim*density((xx+(i-5.0d0-0.5d0)*delta_x/n)&
                  ,(yy+(j-5.0d0-0.5d0)*delta_x/n),(zz+(k-5.0d0-0.5d0)*delta_x/n),rho_0,r0) 
          end do
       end do
   end do
#endif
...
if (r .lt. 0.5d0*r0)then
   !write(*,*),4*rho_0,rho_0*(r0/r)**2,'r < 0.5r0,4rho_0,formule'
...
! write(*,*),zz,'zz P'
  do i=1,n
      do j=1,n
! computing the integral in the middle of the slice, be careful with size of the box!
         integral =integral +delta_x/n*delta_x/n*pressure((xx+(i-5.0d0-0.5d0)*delta_x/n)&
              ,(yy+(j-5.0d0-0.5d0)*delta_x/n),zz,P_0,r0) 
      end do
   end do
#endif
   ...
   do i=1,n
       do j=1,n
          do k=1,n
! computing the integral in the middle of the slice, be careful with size of the box!
             integral =integral +(delta_x/n)**ndim*pressure((xx+(i-5.0d0-0.5d0)*delta_x/n)&
                  ,(yy+(j-5.0d0-0.5d0)*delta_x/n),(zz+(k-5.0d0-0.5d0)*delta_x/n),P_0,r0) 
          end do
       end do
   end do
#endif
...
if (r .lt. 0.5d0*r0)then
   pressure=min(P_0*(r0/r)**(2.0d0*gamma),2.0d0**(2.0d0*gamma)*P_0)
 !  write(*,*),P_0*(r0/r)**(2.0d0*gamma/3),2.0d0**(2.0d0*gamma)*P_0,pressure,'P1,P2,Pmin'
else
   pressure=P_0*(r0/r)**(2.0d0*gamma)
endif
...
!write(*,*),zz,'z vx'
   do i=1,n
      do j=1,n
! computing the integral in the middle of the slice, be careful with size of the box!
         integral =integral +delta_x/n*delta_x/n*velocity_x((xx+(i-5.0d0-0.5d0)*delta_x/n)&
              ,(yy+(j-5.0d0-0.5d0)*delta_x/n),zz,v_inf,R_star)
      end do
   end do
#endif
  ...
  ! write(*,*),'3d integ vx'
   do i=1,n
      do j=1,n
         do k=1,n 
            integral =integral +(delta_x/n)**ndim*velocity_x((xx+(i-5.0d0-0.5d0)*delta_x/n)&
                  ,(yy+(j-5.0d0-0.5d0)*delta_x/n),(zz+(k-5.0d0-0.5d0)*delta_x/n),v_inf,R_star) 
         end do
      end do
   enddo
#endif
...
!write(*,*),zz,'z vy'
   do i=1,n
      do j=1,n
! computing the integral in the middle of the slice, be careful with size of the box!
         integral =integral +delta_x/n*delta_x/n*velocity_y((xx+(i-5.0d0-0.5d0)*delta_x/n)&
              ,(yy+(j-5.0d0-0.5d0)*delta_x/n),zz,v_inf,R_star)
       end do
    end do
#endif
   ...
   do i=1,n
      do j=1,n
         do k=1,n
            integral =integral +(delta_x/n)**ndim*velocity_y((xx+(i-5.0d0-0.5d0)*delta_x/n)&
                  ,(yy+(j-5.0d0-0.5d0)*delta_x/n),(zz+(k-5.0d0-0.5d0)*delta_x/n),v_inf,R_star) 
         end do
      end do
   end do 
#endif
!write(*,*),integral,integral/volume,'integral,vx'
...
!write(*,*),' 3D integ vz '
   ...
   do i=1,n
      do j=1,n
         do k=1,n
            integral =integral +(delta_x/n)**ndim*velocity_z((xx+(i-5.0d0-0.5d0)*delta_x/n)&
                  ,(yy+(j-5.0d0-0.5d0)*delta_x/n),(zz+(k-5.0d0-0.5d0)*delta_x/n),v_inf,R_star) 
         end do
      end do
   end do 
...
!faire un tableau pour avoir les vitesses sur les 3 axes (voire toutes les autres variables d'ailleurs. Voir pour les projections!!!!!
...
!velocity=v_inf*max((1-R_star/r)**beta,0.d0)
return
...
#endif
!write(*,*),r,x,y,z,2*r0,'r,x,y,z,2r0'

fract=compteur/(n**ndim)

!write(*,*),fract,compteur,'fract,compteur'
return
end function fract
