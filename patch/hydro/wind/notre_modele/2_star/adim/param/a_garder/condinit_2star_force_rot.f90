...
subroutine condinit(x,u,dx,nn)
  ...
  real(dp):: delta_x
  real(dp)::a ! physical separation of the system, expressed in a AU. In the simulation the separation is rescaled to 1.
  ...
  DO i=1,nn 
     ...
     IF (rc .lt. 1.2d0*r0_2) THEN
      ...
      ENDIF
        ...
        q(i,iv) =  fract(xx,yy,zz,r0_2,delta_x)*integration_vy(xx,yy,zz,v_inf2,epsilon_star2&
             ,beta,delta_x)+(1.d0-fract(xx,yy,zz,r0_2,delta_x))*q(i,iv)
        q(i,id) =  fract(xx,yy,zz,r0_2,delta_x)*integration_rho(xx,yy,zz,M_dot2,r0_2,delta_x,a&
             ,v_inf2,epsilon_star2,beta)+(1.d0-fract(xx,yy,zz,r0_2,delta_x))*q(i,id)
        q(i,ip) =  fract(xx,yy,zz,r0_2,delta_x)*integration_P(xx,yy,zz,P_2,rho_2,M_dot2,a,r0_2&
           ,v_inf2 ,epsilon_star2,beta,gamma,delta_x)+(1.d0-fract(xx,yy,zz,r0_2,delta_x))*q(i,ip)
...
!values for r=a
 ...
 !positions of the stars
  ...
  x02   = 0.5d0*boxlen+a*cos(2.0D0*3.1415/period *t) !+(a/2.0d0)
  y02   = 0.5d0*boxlen+a*sin(2.0d0*3.1415/period *t)
  z02   = 0.5d0*boxlen
         ...
         !   write(*,*),xcen1,xx(i,1),x01,'xcen,xx,x01'
            ...
            if (rc1 .lt. 1.2d0*r0_1) then
             ...
             uold(ind_cell(i),3)=fract(xcen1,ycen1,zcen1,r0_1,delta_x)*u_mask(i,3)&
              ...
              uold(ind_cell(i),1)=fract(xcen2,ycen2,zcen2,r0_2,delta_x)*u_mask(i,1)&
                    +(1.d0-fract(xcen2,ycen2,zcen2,r0_2,delta_x))*uold(ind_cell(i),1)
              ...
              uold(ind_cell(i),4)= fract(xcen2,ycen2,zcen2,r0_2,delta_x)*u_mask(i,4)&
                   +(1.d0-fract(xcen2,ycen2,zcen2,r0_2,delta_x))*(uold(ind_cell(i),4)&
                   +force(v_inf2,a,epsilon_star2,rho_2,v_2,rc2)*velocity_fluid*dtnew(ilevel))

              uold(ind_cell(i),2)=fract(xcen2,ycen2,zcen2,r0_2,delta_x)*u_mask(i,2)&
                    +(1.d0-fract(xcen2,ycen2,zcen2,r0_2,delta_x))*(uold(ind_cell(i),2)&
                    +xcen2/rc2*dtnew(ilevel)*force(v_inf2,a,epsilon_star2,rho_2,v_2,rc2))
              uold(ind_cell(i),3)=fract(xcen2,ycen2,zcen2,r0_2,delta_x)*u_mask(i,3)&
                    +(1.d0-fract(xcen2,ycen2,zcen2,r0_2,delta_x))*(uold(ind_cell(i),3)&
                    +ycen2/rc2*dtnew(ilevel)*force(v_inf2,a,epsilon_star2,rho_2,v_2,rc2))
#endif
           ...
           elseif(( rc1 .gt. 1.2d0*r0_1) .and. (rc2 .gt. 1.2d0*r0_2))then!
              ...
              +xcen2/rc2*force(v_inf2,a,epsilon_star2,rho_2,v_2,rc2 )*dtnew(ilevel)
             uold(ind_cell(i),3)=uold(ind_cell(i),3)&
              +ycen1/rc1*force(v_inf1,a,epsilon_star1,rho_1,v_1,rc1 )*dtnew(ilevel)&
              +ycen2/rc2*force(v_inf2,a,epsilon_star2,rho_2,v_2,rc2 )*dtnew(ilevel)
...
!conversion according to the value of a
 ...
 ! write(*,*),M_dot2,Mach_2,v_inf2,r0_2,epsilon_star2,a,'Mdot2,Mach2,vinf2,r02,eps2,a'
