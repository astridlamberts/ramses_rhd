...
!run parameters
 ...
 ! r0    = 0.054d0*boxlen_0/boxlen
  ...
  x02   = 0.5d0*(boxlen+0.4d0*boxlen_0/boxlen)
  y02   = 0.5d0*boxlen
  z02   = 0.5d0*boxlen
...
!do i =1,nn
   ...
   !if ( q(i,ndim+2) .gt. 50.0 ) then
   !   write(*,*), i,q(i,ndim+2),'i,p'
   !   endif
!end do
...
if (r .lt. 0.5d0*r0)then
   pressure=min(P_0*(r0/r)**(2.0d0*gamma/3),2.0d0**(2.0d0*gamma)*P_0)
 !  write(*,*),P_0*(r0/r)**(2.0d0*gamma/3),2.0d0**(2.0d0*gamma)*P_0,pressure,'P1,P2,Pmin'
else
   pressure=P_0*(r0/r)**(2.0d0*gamma/3)
endif
