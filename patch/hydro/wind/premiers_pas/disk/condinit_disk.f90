...
!Disk
  ...
  DO i=1,nn ! all the cells
     ...
     IF (rc3 .lt. 2*r01) THEN
        q(i,id) = min(rho_0*(r01/rc),4.d0*rho_0)
...
#if NDIM==2   
        !write(*,*)'on est a 2D'
        IF ((rc .lt. 2*r01).and.(rc .gt. R_star1)) THEN
        q(i,id) = min(rho_0*(r01/rc)**2,4.d0*rho_0)
! a priori pas besoin de la condition!!!!
        q(i,iu) = min(xx/rc*sqrt(G*M/rc),xx/R_star1*sqrt(G*M/R_star2))
