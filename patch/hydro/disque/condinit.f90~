...
subroutine condinit(x,u,dx,nn)
  ...
  real(dp):: v_a,P_a,rho_a,P_amb,rho_amb
  integer :: ivar,ncell
...
! Initialising the disk. Its necessary on the first object. The star with the disk does not move.
  ...
  if (disk_presence .eqv. .true.) then
    DO i=1,nn ! all the cells
       ...
       H=H_0*(rc/rin)**((3.-qu)/2.)
       q(i,id)=max(rho_0*(rin/rc)**pe*exp(-zz**2/2.d0/H**2),rho_amb)
       csound=cs_0*(rin/rc)**(qu/2.0d0)
       q(i,ip)=csound**2*q(i,id)/gamma

!   velocity  not strictly keplerian due to pressure
!       omega=sqrt(GM/(r3d*(r01**2+r3d**2)))
!#if NDIM==2
       omega=sqrt(Gstar*M1/(rc*(r01**2+rc**2))+csound**2*(pe+qu)/(rc**2)) 
!#endif

       if (rc.lt. r01) then
!       write(*,*),rc,omega,'omega_disk'
       endif
       q(i,iu)=-omega*yy
       q(i,iv)=omega*xx
 ...
 rho_a = Mdot*a1/(2.0d0*pi*v_a)
  ...
  DO i=1,nn ! all the cells
     ...
     IF (rc .lt. 1.2d0*r0) THEN
        ...
        q(i,ip) =  frac(xx,yy,zz,r0,dx)*int_P(xx,yy,zz,P_a,rho_a,Mdot,r0,vinf,rstar,beta,gamma,dx) &
             &+(1.d0-frac(xx,yy,zz,r0,dx))*q(i,ip)
        if (nvar  .gt. ip) then
     ...
     IF (rc .lt. 1.2d0*r0) THEN
       ...
       q(i,iv) =  frac(xx,yy,zz,r0,dx)* int_vy(xx,yy,zz,vinf,rstar,beta,dx)+(1.d0-frac(xx,yy,zz,r0,dx))*q(i,iv)
       q(i,id) =  frac(xx,yy,zz,r0,dx)*int_rho(xx,yy,zz,Mdot,r0,dx,vinf,rstar,beta)+(1.d0-frac(xx,yy,zz,r0,dx))*q(i,id)
       q(i,ip) =  frac(xx,yy,zz,r0,dx)*int_P(xx,yy,zz,P_a,rho_a,Mdot,r0,vinf,rstar,beta,gamma,dx)+(1.d0-frac(xx,yy,zz,r0,dx)) &
           & *q(i,ip)
       if (nvar  .gt. ip) then
          q(i,ip+index_pas_scal)=frac(xx,yy,zz,r0,dx)
       endif
...
subroutine init_puls(q,x,nn,vinf,r0,Mdot,Mach,x0,y0,z0,index_pas_scal) 
  ...
  real(dp):: rho_in,vwind,omega,csound,h,r3d
  real(dp)::cs,vrot,int_vx ,int_vy,int_vz,v
 ...
 rho_in=Mdot*a1/(2.0d0*pi*r0*vinf)
#endif

  P_a = vinf**2* rho_a/(Mach**2*gamma)

  !variables at edge of the mask, where the disk starts, assumes constant velocity in the wind
  rho_in=Mdot*a1**2/(4.0d0*3.1415*(r0)**2*vinf)
...
zz = 0.0d0  
  ...
  DO i=1,nn 
     ...
     yy=x(i,2)-y0
     rc=sqrt(xx**2+yy**2)
     csound=cs_0*(rin/r0)**(qu/2.)         !!!!!!!!!!!!a verifier!!!

#if NDIM == 3 
     zz=x(i,3)-z0
     r3D=sqrt(xx**2+yy**2+zz**2)
     cs=cs_0*(rin/r0)**(qu/2.)
     IF (r3d .lt. 1.2d0*r0) THEN
        H=H_0*(rc/rin)**((3.-qu)/2.)
        q(i,id)=rho_0/100*(rin/r0)**pe
        q(i,ip)=cs**2*q(i,id)
        ! has to be identical to 3D case
        v=r3D*sqrt(Gstar*M1/(r3D**2+r0**2)**(3./2.))
        vwind=int_vx(xx,yy,zz,vinf,r0,beta1,dx)
        vrot=v*yy/rc
        q(i,iu) =  frac(xx,yy,zz,r0,dx)* (vwind+vrot)+(1.d0-frac(xx,yy,zz,r0,dx))*q(i,iu)
        vwind=int_vy(xx,yy,zz,vinf,r0,beta1,dx)
        vrot=-v*xx/rc
        q(i,iv) =  frac(xx,yy,zz,r0,dx)* (vwind+vrot)+(1.d0-frac(xx,yy,zz,r0,dx))*q(i,iv)
        q(i,iw) =  frac(xx,yy,zz,r0,dx)* int_vz(xx,yy,zz,vinf,r0,beta1,dx)+(1.d0-frac(xx,yy,zz,r0,dx))*q(i,iw)
...
!        omega=0.0d0
!        if (disk_presence .eqv. .true.) then
!          omega=sqrt(Gstar*M1/(r3d*(r0**2+r3d**2)))!! no pressure gradient correction yet
!        endif  
!     H=H_0*(rc/rin)**((3.-qu)/2.)
!     q(i,id)=   frac(xx,yy,zz,r0,dx)* rho_0*(rin/r0)**pe +(1.d0-frac(xx,yy,zz,r0,dx))*q(i,id)    
!     q(i,ip)=csound**2*q(i,id)/gamma ! a changer si on veut ad.
!     vwind=vinf*xx/r3d!int_vx(xx,yy,zz,vinf,r0,beta,dx)
!     q(i,iu) =  frac(xx,yy,zz,r0,dx)* (vwind-omega*yy)+(1.d0-frac(xx,yy,zz,r0,dx))*q(i,iu)
!     vwind=vinf*yy/r3d!int_vy(xx,yy,zz,vinf,r0,beta,dx)
!     q(i,iv) =  frac(xx,yy,zz,r0,dx)* (vwind+omega*xx)+(1.d0-frac(xx,yy,zz,r0,dx))*q(i,iv)
!     q(i,iw) =  frac(xx,yy,zz,r0,dx)* vinf*zz/r3D+(1.d0-frac(xx,yy,zz,r0,dx))*q(i,iw)
...
! The density has to be continuous across the edge of the disk.
        if (disk_presence .eqv..true.) then
           q(i,id)= frac(xx,yy,zz,r0,dx)* rho_0*(rin/r0)**pe +(1.d0-frac(xx,yy,zz,r0,dx))*q(i,id) 
           q(i,ip)=csound**2*q(i,id)/gamma
           omega=sqrt(Gstar*M1/(rc*(r0**2+rc**2))+csound**2*(pe+qu)/(rc**2)) 
           q(i,iu)=-omega*yy
           q(i,iv)=+omega*xx

       !    write(*,*),rc,omega,'omega ini'
           if ((q(i,iu)*xx+q(i,iv)*yy).gt. 1.0e-14) then
!              write(*,*)q(i,iu)*xx+q(i,iv)*yy,'vitesse radiale star'
           endif
     else   
           q(i,id)=   frac(xx,yy,zz,r0,dx)* rho_in+(1.d0-frac(xx,yy,zz,r0,dx))*q(i,id) 
           q(i,ip)=csound**2*q(i,id)/gamma
           vwind=vinf*xx/rc!int_vx(xx,yy,zz,vinf,r0,beta,dx)
           q(i,iu) =  frac(xx,yy,zz,r0,dx)* vwind+(1.d0-frac(xx,yy,zz,r0,dx))*q(i,iu)
           vwind=vinf*yy/rc!int_vy(xx,yy,zz,vinf,r0,beta,dx)
           q(i,iv) =  frac(xx,yy,zz,r0,dx)*vwind+(1.d0-frac(xx,yy,zz,r0,dx))*q(i,iv)
       endif    
 ...
 !positions of the bodies
  ...
  if (rotation .eqv. .false.)then
     ...
     if (ilevel .eq. nlevelmax) then
        ...
        ! take into acccount the softening length.? No because the stars don't get that close?
        ! The leapfrog is called only for finelevels otherwise there are to many updates
        !write(*,*),vx01,vy01,vx02,vy02
        call leapfrog(x01,y01,z01,vx01,vy01,vz01,x02,y02,z02,vx02,vy02,vz02,Gstar,M1,M2bis,dtnew(ilevel))
     endif
  endif
...
! a regrouper avec le reste du 3D?
         ...
         if (rc1 .lt. 1.2d0*r01) then
            ...
            if (type1=='star')then 
               call reset_star(xc1,yc1,zc1,r01,dx,u_mask,i,ind_cell(i),ilevel,f1,vinf1,rstar1,rho1,v1,rc1)!
            else if (type1=='puls')then
...
!            case where there is only 1 body
         ...
         elseif((rc1 .gt. 1.2d0*r01) .and. (rc2 .gt. 1.2d0*r02) .and. ((type1 == 'star')  &
              &     .or. (type2 =='star')))then
            !ambient medium
...
subroutine init2_puls(x,u,dx,nn,x0,y0,z0,r0,Mdot,vinf,Mach,delta_x,index_pas_scal)
  ...
  real(dp)::a1=1.0d0
  integer ::index_pas_scal
  real(dp)::csound,r3d,rho_in,H,omega,vwind
  real(dp)::cs,v,vrot,int_vx,int_vy,int_vz
...
!faudra voir si tous les use sont utiles
  ...
  rho_in=Mdot/(4.0d0*3.1415*(a*r0)**2*vinf)
  ...
  DO i=1,nn ! all the cells
     ...
     IF (r3d .lt. 1.2d0*r0) THEN
          ...
          vrot=-v*xx/rc
          q(i,iv) =  vwind+vrot
          q(i,iw) =  int_vz(xx,yy,zz,vinf,r0,beta1,dx)



!     r3d=sqrt(xx**2+yy**2+zz**2)
!     if (disk_presence .eqv. .true.)then
!        omega=sqrt(Gstar*M1/(r3d*(r0**2+r3d**2))) ! non pressure gradient correction yet!!!!)+csound**2*(pe+qu)/(rc**2)) 
!     endif
!     IF (r3d .lt. 1.2d0*r0) THEN
!        H=H_0*(rc/rin)**((3.-qu)/2.)
!        q(i,id)=rho_0*(rin/r0)**pe
!        q(i,ip)=csound**2*q(i,id)/gamma
!        vwind=vinf*xx/r3d!int_vx(xx,yy,zz,vinf,r0,beta,dx)
!        q(i,iu)=-omega*yy+vwind
!        vwind=vinf*yy/r3d!int_vy(xx,yy,zz,vinf,r0,beta,dx)
!        q(i,iv)=omega*xx+vwind
!        q(i,iw) = vinf*zz/r3d! int_vz(xx,yy,zz,vinf,r0,beta,dx)
       ! Convert primitive to conservative variables
        ! density -> density
        u(i,id)=q(i,id)
        ! velocity -> momentum
...
! The density has to be continuous across the edge of the disk.
        if (disk_presence .eqv..true.) then
           q(i,id)= rho_0*(rin/r0)**pe !frac(xx,yy,zz,r0,dx)* rho_0*(r_0/r0)**pe +(1.d0-frac(xx,yy,zz,r,dx))*q(i,id) 
           q(i,ip)=csound**2*q(i,id)/gamma
           omega=sqrt(Gstar*M1/(rc*(r0**2+rc**2))+csound**2*(pe+qu)/(rc**2)) 
!           write(*,*),rc,omega
           q(i,iu)=-omega*yy
...
use hydro_parameters ! pour gamma
implicit none

 logical::nml_ok
 real::omega_0

 namelist/first_body/type1,Mdot1,Mach1,vinf1,r01,rstar1,beta1,f1,M1
 namelist/second_body/type2,Mdot2,Mach2,vinf2,r02,rstar2,beta2,f2,M2
 namelist/disk/disk_presence,rho_0,pe,qu,HR,rin
...
real (kind=8)::r,r1,r2,dthetadt,M2bis
real (kind=8)::period
...
!if (myid .eq. 1) then
!   period=sqrt((4.0d0*pi**2*a**3)/(Gstar*(M1+M2)))
!   write(*,*), 'the period of the system is ', period,'years'
!endif
!#endif
...
! if there is a disk the Be star doesnt turn, it might give an unstable disk.
if (disk_presence .eqv. .true.) then
   M2bis=0.0d0
else
   M2bis=M2
...
x01=xcen+M2bis/(M1+M2bis)*a1*(1-exc)*(cos(node)*cos(peri+theta)-sin(node)*sin(peri+theta)*cos(inc))
y01=ycen+M2bis/(M1+M2bis)*a1*(1-exc)*(sin(node)*cos(peri+theta)+cos(node)*sin(peri+theta)*cos(inc))
z01=zcen+M2bis/(M1+M2bis)*a1*(1-exc)*sin(peri+theta)*sin(inc)
...
ax1=-Gstar*M2bis/r**2*(x01-x02)/r
ay1=-Gstar*M2bis/r**2*(y01-y02)/r
az1=-Gstar*M2bis/r**2*(z01-z02)/r

ax2=-Gstar*M1/r**2*(x02-x01)/r
ay2=-Gstar*M1/r**2*(y02-y01)/r
az2=-Gstar*M1/r**2*(z02-z01)/r 

dthetadt=sqrt(a1*(1-exc**2)*Gstar*(M1+M2bis)/(r**4))
...
!assumes rotation around common center of mass.
