...
!     qpuls=0.!1.4/M1!10.!gravity_params(4)   ! Pulsar to star mass ratio
     ...
     !     yc=0.5*boxlen
     !     zc=0.5*boxlen
!     estar=r01!2.0d0*boxlen/2**(nlevelemax)
!     epuls=r02!2.0d0*boxlen/2**(nlevelemax)
...
!        rrstar=sqrt(rxs**2+rys**2+rzs**2+estar**2)
  ...
  !        ff=sqrt(Gstar*100*m1*(1.0-qpuls)/rrstar**3)
  !        write(*,*),x01,y01,rxs
...
!        if(ndim>2)f(i,3)=-rzs*ff
  !        write(*,*),gstar,m1,qpuls
        !force due to the pulsar
 !       if(ndim>0)f(i,1)=f(i,1)-m2*qpuls*rxp/rrpuls**3
 !       if(ndim>1)f(i,2)=f(i,2)-m2*qpuls*ryp/rrpuls**3
 !       if(ndim>2)f(i,3)=f(i,3)-m2*qpuls*rzp/rrpuls**3
!     end do
!  end if
