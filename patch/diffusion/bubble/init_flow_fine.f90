...
subroutine init_flow_fine(ilevel)
  ...
  real(dp)::dx,rr,vx,vy,vz,ek,ei,pp,scale_T2,xx1,xx2,xx3,dx_loc,scale
  ...
  if(ok_file)then
     ...
     do ivar=1,nvar_in
        ...
        if(cosmo)then
           ...
           if(ivar==2)init_array=dfact(ilevel)*vfact*dx_loc/dxini(ilevel)*init_array
           if(ivar==3)init_array=dfact(ilevel)*vfact*dx_loc/dxini(ilevel)*init_array
           if(ivar==4)init_array=dfact(ilevel)*vfact*dx_loc/dxini(ilevel)*init_array
        endif
        ! Loop over cells
        do ind=1,twotondim
     ...
     deallocate(init_array,init_plane) 

     !---------------------------------------------------
     ! For cosmology runs: compute initial pressure
     !---------------------------------------------------
     if(cosmo)then
        ! Scaling factor (physical units to cosmological units)
        scale_T2 = (100.*boxlen*1.d5/aexp)**2*mH/kB
...
subroutine region_condinit(x,q,dx,nn)
  ...
  q(1:nn,1)=smallr
  if(ndim>0)q(1:nn,2)=0.0d0
#if NDIM>1
  if(ndim>1)q(1:nn,3)=0.0d0
#endif
#if NDIM>2
  if(ndim>2)q(1:nn,4)=0.0d0
#endif
  ...
  do k=1,nregion
     ...
     if(region_type(k) .eq. 'square')then
        ...
        do i=1,nn
           ...
           if(ndim>0)xn=2.0d0*abs(x(i,1)-x_center(k))/length_x(k)
#if NDIM>1
           if(ndim>1)yn=2.0d0*abs(x(i,2)-y_center(k))/length_y(k)
#endif
#if NDIM>2
           if(ndim>2)zn=2.0d0*abs(x(i,3)-z_center(k))/length_z(k)
#endif
           ...
           if(r<1.0)then
              q(i,1)          =d_region(k)
              if(ndim>0)q(i,2)=u_region(k)
#if NDIM>1
              if(ndim>1)q(i,3)=v_region(k)
#endif
#if NDIM>2
              if(ndim>2)q(i,4)=w_region(k)
#endif
              q(i,ndim+2)     =p_region(k)
              q(i,ndim+3)     =t_region(k)
           end if
        end do
     end if
     ...
     if(region_type(k) .eq. 'point')then
        ...
        do i=1,nn
           ...
           if(ndim>0)xn=max(1.0-abs(x(i,1)-x_center(k))/dx,0.0_dp)
#if NDIM>1
           if(ndim>1)yn=max(1.0-abs(x(i,2)-y_center(k))/dx,0.0_dp)
#endif
#if NDIM>2
           if(ndim>2)zn=max(1.0-abs(x(i,3)-z_center(k))/dx,0.0_dp)
#endif
           r=xn*yn*zn
           ...
           q(i,ndim+3)=q(i,ndim+3)+t_region(k)*r/vol
        end do
     end if
  end do
