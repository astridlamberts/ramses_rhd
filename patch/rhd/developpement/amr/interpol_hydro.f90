...
subroutine upl(ind_cell,ncell)
  ...
  ! interpol_tar=0: use rho, rho u and rho epsilon
  !---------------------------------------------------------------------
  ...
  do ivar=1,nvar     
     ...
     if(interpol_var==1.and.ivar==5)then
...
subroutine interpol_hydro(u1,g1,u2,g2,nn)
  ...
  real(dp) :: D,M,E,Mx,My,Mz,v2,Xsi,R,qp,qd,vx,vy,vz
  real(dp) :: Dr,Mr,Er,Mxr,Myr,Mzr
  ...
  do ind=1,twotondim
     do idim=1,ndim
        do i=1,nn
           ...
           Dr = u_record(i,ind,1) 
           ! Compute momentum
           Mxr=u_record(i,ind,2) ; Myr=u_record(i,ind,3) ; Mzr=u_record(i,ind,4)
           Mr = sqrt(Mxr**2+Myr**2+Mzr**2)
           ! Compute total energy 
           Er = u_record(i,ind,5)
...
!              write(*,*),M,E,Mr,Er,'prolo',i,ind
            !  write (*,*) ' switch first order interpol_hydro'
            !  do ivar=1,nvar
            !     write(*,*),u2(i,ind,ivar),u_record(i,ind,ivar)
...
!           enddo
 ...
 if (compteur .ne. 0) then
  ...
  !           write(*,*),'back to first order'
             ...
             if (M>E .or. (E**2<M**2+D**2).or.(E<0)) then
 !               write(*,*),M,E,Mr,Er,'merde prolo',i,ind,nn
                qd = smallr
                qp = smallr

                lor=1./1.e-4
                entho=1.d0+gamma/(gamma-1.d0)*qp/qd

                vx = Mx/M*(lor**2-1.d0)**0.5/lor
                vy = My/M*(lor**2-1.d0)**0.5/lor
                vz = Mz/M*(lor**2-1.d0)**0.5/lor

                u2(i,ind,1)=qd*lor
                u2(i,ind,2)=qd*lor**2*entho*vx
                u2(i,ind,3)=qd*lor**2*entho*vy
                u2(i,ind,4)=qd*lor**2*entho*vz
                u2(i,ind,5)=qd*lor**2*entho - qp
