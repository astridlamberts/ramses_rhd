...
subroutine init_puls(q,x,nn,vinf,r0,Mdot,Mach,x0,y0,z0,index_pas_scal) 
  ...
  real(dp)::rho_a,P_A,rho_m,p_m,vinfm ! variables for r =a, r=r0
  real(dp),dimension(1:nvector,1:nvar)::q   ! Primitive variables
...
#if NDIM==3
  !variables at edge of the mask
  rho_m = Mdot/(4.0d0*pi*vinf*r0**2)
  P_m = P_a*(rho_a/rho_m)**(-gamma)

  vinfm=sqrt(vinf**2-P_m/rho_m*gamma/(gamma-1.d0))
#endif
...
zz = 0.0d0  
  ...
  DO i=1,nn 
     ...
     rc=sqrt(xx**2+yy**2+zz**2)
     IF (rc .lt. 1.2d0*r0) THEN
        q(i,iu) = frac(xx,yy,zz,r0,dx)*vinfm*(xx/rc)+(1.0d0-frac(xx,yy,zz,r0,dx))*q(i,iu)
        q(i,iv) = frac(xx,yy,zz,r0,dx)*vinfm*(yy/rc)+(1.0d0-frac(xx,yy,zz,r0,dx))*q(i,iv)
        q(i,iw) = frac(xx,yy,zz,r0,dx)*vinfm*(zz/rc)+(1.0d0-frac(xx,yy,zz,r0,dx))*q(i,iw)
        q(i,id) = frac(xx,yy,zz,r0,dx)*int_rhoP(xx,yy,zz,Mdot,r0,dx,vinfm)+(1.d0-frac(xx,yy,zz,r0,dx))*q(i,id)
        q(i,ip) = frac(xx,yy,zz,r0,dx)*int_pP(xx,yy,zz,P_a,rho_a,Mdot,r0,vinfm,gamma,dx)+(1.d0-frac(xx,yy,zz,r0,dx))*q(i,ip)
        if (nvar  .gt. ip) then      
           q(i,ip+index_pas_scal) = frac(xx,yy,zz,r0,dx)
        endif
     ENDIF
...
!           write(*,*),x01,y01,vx01,vy01,x02,y02,vx02,vy02
 ...
 ! write(*,*),vinf,P_m,rho_m
  ...
  DO i=1,nn ! all the cells
     ...
     IF (rc .lt. 1.2d0*r0) THEN
        ...
        q(i,iv) = vinfm*(yy/rc)
        q(i,iw) = vinfm*(zz/rc)
        q(i,id) = int_rhoP(xx,yy,zz,Mdot,r0,delta_x,vinfm)
        q(i,ip) = int_pP(xx,yy,zz,P_a,rho_a,Mdot,r0,vinfm,gamma,delta_x)
        if (nvar .gt. ip) then
           q(i,ip+1)=0.0d0
           q(i,ip+2)=0.0d0
           q(i,ip+index_pas_scal)=1.0d0
