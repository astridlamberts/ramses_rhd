...
!              polytropic relativistic gas dynamics using Piecewise Linear Method  with
!              hll and hllc riemann solver
!              with various slope limiters
!              This routine is inspired by the hydro/plmde scheme 
!              
!
!  inputs/outputs

!  q           => (const)  input state (primitive variables)
!  iu1,iu2     => (const)  first and last index of input and output array,
!  ju1,ju2     => (const)  cell centered,    
!  ku1,ku2     => (const)  including buffer cells.
!  dq          => (const)  tvd slopes (minmod or moncen)

!  dx          => (const
!  dt          => (const)  time step
!  ngrid       => (const)  number of sub-grids
!  qm, qp      => (output) left and right vectors
!
!  q = (\rho, u, v,  w, P)
!  Note that here we have 3 components for v  whatever ndim.
!
!  This routine was written by Astrid Lamberts
! ----------------------------------------------------------------
!
! ----------------------------------------------------------------
subroutine tracex(q,dq,c, qm,qp,dx,dt,ngrid)
  ...
  real(dp)::ni,h,lor,velsq,smallp, rp , rm,entho,vtot2,coef
...
subroutine tracexy(q,dq,c,qm,qp,dx,dy,dt,ngrid)
  ...
  real(dp)::drx, dux, dvx, dwx, dpx, dax,sax,say
  ...
  real(dp)::eta,b,vtot,lor,vtot2,coef,rm,rp
...
!              dpx = dq(l,i,j,k,ip,1)
              ...
              qp(l,i,j,k,1,2) = r + (apright+ azrxright+amright)
              ...
              qm(l,i,j,k,1,2) = r + (apleft+ azrxleft+amleft)
