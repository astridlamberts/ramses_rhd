...
!         if (nvar  .gt. 5) then 
!            if (uold(ind_cell(i),6) .lt. 1.0e-4 )then 
!               uold(ind_cell(i),6)=0
!            endif
!         endif
      end do
...
!           write(*,*),uold(ind_cell,1),u_mask(ind_cell,1),frac(xc,yc,zc,r0,delta_x),ind_cell,'reset_star'
     enddo   
...
!  endif
  ...
  DO i=1,nn ! all the cells
     ...
     IF (rc .lt. 1.2d0*r0) THEN
        ...
        do ivar=6,nvar
           u(1:nn,ivar)=q(1:nn,1)*q(1:nn,ivar)*lor
!           write(*,*),q(i,6),u(i,6),i,index_pas_scal
        end do
!        if ((u(i,6) .ne. 0) .or. (u(i,7) .ne. 0)) then
