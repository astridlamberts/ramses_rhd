...
subroutine hydro_refine(ug,um,ud,ok,nn)
  ...
  real(dp)::dg,dm,dd,pg,pm,pd,vg,vm,vd,cg,cm,cd,error,lorg,lorm,lord
     ...
     !convert to primitive variables
!  write(*,*),'hydro_refine avant 1'
  call ctoprimbis(ug,nn,qg)
 ! write(*,*),'hydro_refine avant 2'
  call ctoprimbis(um,nn,qm)
!  write(*,*),'hydro_refine avant 3'
  call ctoprimbis(ud,nn,qd)
!  write(*,*),'hydro_refine apres 3'
...
!     if (M>E) then
!        write (*,*) 'M>E ctoprim 2',D,M,E,'ctoprimbis'
!     endif
     if (E**2<M**2+D**2) then
...
!        lor=1./1.e-4
!        entho=1.d0+gamma/(gamma-1.d0)*q(k,5)/q(k,1)
        
!        q(k,2) = Mx/M*(lor**2-1.d0)**0.5/lor
!        q(k,3) = My/M*(lor**2-1.d0)**0.5/lor
!        q(k,4) = Mz/M*(lor**2-1.d0)**0.5/lor
        
        
!        uu(k,1)=q(k,1)*lor
!        uu(k,2)=q(k,1)*lor**2*entho*q(k,2)
!        uu(k,3)=q(k,1)*lor**2*entho*q(k,3)
!        uu(k,4)=q(k,1)*lor**2*entho*q(k,4)
!        uu(k,5)=q(k,1)*lor**2*entho-q(k,5)
!        ! Compute density
!        D = uu(k,1) 
        ! Compute momentum
!        Mx=uu(k,2) ; My=uu(k,3) ; Mz=uu(k,4)
...
!     else
     ...
     q(k,5)=max(1d-18,(gamma-1.d0)/gamma*Xsi)
