...
subroutine trace2d(q,dq,qm,qp,dx,dy,dt,ngrid)
  ...
  real(dp)    :: lor,lorsq,h,kappa,chi,f,ni,cs2,vtot,velsq,entho
  real(dp)    :: a11,a12,a15,a22,a25,a32,a33,a35,a42,a44,a45,a52,a55
     ...
     do j = jlo, jhi
        do i = ilo, ihi
           do l = 1, ngrid
              ...
              lor=(1d0-u**2-v**2-w**2)**(-1./2.)
              D=r*lor
              h=1+gamma/(gamma-1d0)*p/r
              Mx=u*lor**2*h*r
              My=v*lor**2*h*r
              Mz=w*lor**2*h*r
              E=lor**2*h*r-p
              ...
              if ((D<0).or.(E**2<M**2+D**2) .or. E<0 .or. M>E) then
                 write(*,*),D,Mx,My,Mz,E,M,'center'
              endif
              ...
              lor=(1d0-qp(l,i,j,k,2,1)**2-qp(l,i,j,k,3,1)**2-qp(l,i,j,k,4,1)**2)**(-1./2.)
              D=qp(l,i,j,k,1,1)*lor
              h=1+gamma/(gamma-1d0)*qp(l,i,j,k,5,1)/qp(l,i,j,k,1,1)
              Mx=qp(l,i,j,k,2,1)*lor**2*h*qp(l,i,j,k,1,1)
              My=qp(l,i,j,k,3,1)*lor**2*h*qp(l,i,j,k,1,1)
              Mz=qp(l,i,j,k,4,1)*lor**2*h*qp(l,i,j,k,1,1)
              E=lor**2*h*qp(l,i,j,k,1,1)-qp(l,i,j,k,5,1)
              M=sqrt(Mx**2+My**2+Mz**2)
              if ((D<0).or.(E**2<M**2+D**2) .or. E<0 .or. (E<M)) then
                 write(*,*),D,Mx,My,Mz,E,M,'right'
              ...
              lor=(1d0-qm(l,i,j,k,2,1)**2-qm(l,i,j,k,3,1)**2-qm(l,i,j,k,4,1)**2)**(-1./2.)
              D=qm(l,i,j,k,1,1)*lor
              h=1+gamma/(gamma-1d0)*qm(l,i,j,k,5,1)/qm(l,i,j,k,1,1)
              Mx=qm(l,i,j,k,2,1)*lor**2*h*qm(l,i,j,k,1,1)
              My=qm(l,i,j,k,3,1)*lor**2*h*qm(l,i,j,k,1,1)
              Mz=qm(l,i,j,k,4,1)*lor**2*h*qm(l,i,j,k,1,1)
              E=lor**2*h*qm(l,i,j,k,1,1)-qm(l,i,j,k,5,1)
              M=sqrt(Mx**2+My**2+Mz**2)
              if ((D<0).or.(E**2<M**2+D**2) .or. E<0 .or. (E<M)) then
                 write(*,*),D,Mx,My,Mz,E,M,'left'
                 qp(l,i,j,k,ir,:)=r
                 qp(l,i,j,k,iu,:)=u
                 qp(l,i,j,k,iv,:)=v
                 qp(l,i,j,k,iw,:)=w
              ...
              lor=(1d0-qp(l,i,j,k,2,2)**2-qp(l,i,j,k,3,2)**2-qp(l,i,j,k,4,2)**2)**(-1./2.)
              D=qp(l,i,j,k,1,2)*lor
              h=1+gamma/(gamma-1d0)*qp(l,i,j,k,5,2)/qp(l,i,j,k,1,2)
              Mx=qp(l,i,j,k,2,2)*lor**2*h*qp(l,i,j,k,1,2)
              My=qp(l,i,j,k,3,2)*lor**2*h*qp(l,i,j,k,1,2)
              Mz=qp(l,i,j,k,4,2)*lor**2*h*qp(l,i,j,k,1,2)
              E=lor**2*h*qp(l,i,j,k,1,2)-qp(l,i,j,k,5,2)
              M=sqrt(Mx**2+My**2+Mz**2)
              if ((D<0).or.(E**2<M**2+D**2) .or. E<0 .or. (E<M)) then
                 write(*,*),D,Mx,My,Mz,E,M,'top'
                 qp(l,i,j,k,ir,:)=r
                 qp(l,i,j,k,iu,:)=u
                 qp(l,i,j,k,iv,:)=v
                 qp(l,i,j,k,iw,:)=w
              ...
              lor=(1d0-qm(l,i,j,k,2,2)**2-qm(l,i,j,k,3,2)**2-qm(l,i,j,k,4,2)**2)**(-1./2.)
              D=qm(l,i,j,k,1,2)*lor
              h=1+gamma/(gamma-1d0)*qm(l,i,j,k,5,2)/qm(l,i,j,k,1,2)
              Mx=qm(l,i,j,k,2,2)*lor**2*h*qm(l,i,j,k,1,2)
              My=qm(l,i,j,k,3,2)*lor**2*h*qm(l,i,j,k,1,2)
              Mz=qm(l,i,j,k,4,2)*lor**2*h*qm(l,i,j,k,1,2)
              E=lor**2*h*qm(l,i,j,k,1,2)-qm(l,i,j,k,5,2)
              M=sqrt(Mx**2+My**2+Mz**2)
              if ((D<0).or.(E**2<M**2+D**2) .or. E<0 .or. (E<M)) then
                write(*,*),D,Mx,My,Mz,E,M,'bottom'
              ...
              lor=(1d0-qp(l,i,j,k,2,3)**2-qp(l,i,j,k,3,3)**2-qp(l,i,j,k,4,3)**2)**(-1./2.)
              D=qp(l,i,j,k,1,3)*lor
              h=1+gamma/(gamma-1d0)*qp(l,i,j,k,5,3)/qp(l,i,j,k,1,3)
              Mx=qp(l,i,j,k,2,3)*lor**2*h*qp(l,i,j,k,1,3)
              My=qp(l,i,j,k,3,3)*lor**2*h*qp(l,i,j,k,1,3)
              Mz=qp(l,i,j,k,4,3)*lor**2*h*qp(l,i,j,k,1,3)
              E=lor**2*h*qp(l,i,j,k,1,3)-qp(l,i,j,k,5,3)
              M=sqrt(Mx**2+My**2+Mz**2)
              if ((D<0).or.(E**2<M**2+D**2) .or. E<0 .or. (E<M)) then
                write(*,*),D,Mx,My,Mz,E,M,'front'
              ...
              lor=(1d0-qm(l,i,j,k,2,3)**2-qm(l,i,j,k,3,3)**2-qm(l,i,j,k,4,3)**2)**(-1./2.)
              D=qm(l,i,j,k,1,3)*lor
              h=1+gamma/(gamma-1d0)*qm(l,i,j,k,5,3)/qm(l,i,j,k,1,3)
              Mx=qm(l,i,j,k,2,3)*lor**2*h*qm(l,i,j,k,1,3)
              My=qm(l,i,j,k,3,3)*lor**2*h*qm(l,i,j,k,1,3)
              Mz=qm(l,i,j,k,4,3)*lor**2*h*qm(l,i,j,k,1,3)
              E=lor**2*h*qm(l,i,j,k,1,3)-qm(l,i,j,k,5,3)
              M=sqrt(Mx**2+My**2+Mz**2)
              if ((D<0).or.(E**2<M**2+D**2) .or. E<0 .or. (E<M)) then
                 write(*,*),D,Mx,My,Mz,E,M,'back'
...
! a voir
  ...
  do k = ku1, ku2
     do j = ju1, ju2
        do i = iu1, iu2
           do l = 1 ,ngrid
!              write(*,*),uin(l,i,j,k,1),uin(l,i,j,k,2),uin(l,i,j,k,5),l,i,j,k
             ! Compute density
             D = uin(l,i,j,k,1) 
             ! Compute momentum
...
!             if (M>E) then
!                write (*,*) 'M>E',l,D,M,E ,'ctoprim'
!             endif
!             if ((E**2<M**2+D**2).or.(E<0).or. D<0) then
...
!                lor=1./1.e-4
!                entho=1.d0+gamma/(gamma-1.d0)*q(l,i,j,k,5)/q(l,i,j,k,1)

!                q(l,i,j,k,2) = Mx/M*(lor**2-1.d0)**half/lor
!                q(l,i,j,k,3) = My/M*(lor**2-1.d0)**half/lor
!                q(l,i,j,k,4) = Mz/M*(lor**2-1.d0)**half/lor

!                uin(l,i,j,k,1)=q(l,i,j,k,1)*lor
 !               uin(l,i,j,k,2)=q(l,i,j,k,1)*lor**2*entho*q(l,i,j,k,2)
!                uin(l,i,j,k,3)=q(l,i,j,k,1)*lor**2*entho*q(l,i,j,k,3)
!                uin(l,i,j,k,4)=q(l,i,j,k,1)*lor**2*entho*q(l,i,j,k,4)
!                uin(l,i,j,k,5)=q(l,i,j,k,1)*lor**2*entho - q(l,i,j,k,5)
...
!             endif
             ...
             q(l,i,j,k,5)=max(1d-18,(gamma-1.d0)/gamma*Xsi)
             if ((q(l,i,j,k,1)<0.d0).or.(q(l,i,j,k,5)<0.d0).or.E<0.d0) then
                write(*,*) 'negative pressure or density ctoprim'
                write(*,*),q(l,i,j,k,1:5),'prim'
                write(*,*),D,Mx,My,Mz,E,'cons'
                stop
             endif
