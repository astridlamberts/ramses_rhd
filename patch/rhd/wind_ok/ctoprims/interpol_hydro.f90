...
subroutine upl(ind_cell,ncell)
  ...
  real(dp),dimension(1:nvector),save::getx,eint,getxE
        ...
        if (ivar .ne. 5) then
           ...
           do ind_son=1,twotondim
              ...
              do i=1,ncell
                 ...
                 !added
                 getxE(i)=getxE(i)+uold(ind_cell_son(i),5)
!              end do
              ...
              if (M>E) then
!                 write (*,*) 'M>E',i,D,M,E ,'interpol_hydro restriction i'
              endif
              if ((E**2<M**2+D**2).or.(E<0)) then
                 
!                 write (*,*) 'Switch...interpol_hydro restriction i'
!                 write(*,*),D,M,E
              endif
              enddo
           end do
           ...
           do i=1,ncell
              ...
              uold(ind_cell(i),5)=getxE(i)/dble(twotondim)
!           end do
           
           ! test for energy normal if pbes...
              ...
              !Method from Mignone,McKinney,2007. Same as BS2011 except one uses E'=U-D and u^2=Lor^2*v^2
              if (M>E) then
   !              write (*,*) 'M>E',i,D,M,E ,'interpol_hydro restriction ii'
              endif
              if ((E**2<M**2+D**2).or.(E<0)) then
                 
!                 write (*,*) 'Switch...interpol_hydro restriction ii'
           ...
           do ind_son=1,twotondim
              ...
              do i=1,ncell
                 ...
                 if (D<0) then
   !                 write(*,*),'D<0 in interpol_hydro'
                    D=small_bigD
                    uold(ind_cell_son(i),1)=D
                 endif
                 if (E<0) then
                    E=sqrt(M**2+d**2+1d-8)
  !                  write(*,*),'E<0 in interpol_hydro'
                    uold(ind_cell_son(i),5)=E
                 endif
        
                 if (E**2<M**2+D**2) then
                    
!                    write (*,*) 'E^2<M^2+D^2 interpol_hydro'
!                    write(*,*),E,'before'
                    E=sqrt(M**2+d**2+1d-8)
                    uold(ind_cell_son(i),5)=E
 !                   write(*,*),E,'after',E-sqrt(M**2+d**2)
                 endif
                 
!             if (M>E) then
!                    write (*,*) 'M>E',i,D,M,E ,'interpol_hydro restriction'
!                 endif
!                 if ((E**2<M**2+D**2).or.(E<0)) then
                    
!                    write (*,*) 'Switch...interpol_hydro restriction'
                    
!                    qd = smallr
!                    qp = smallr
                    
!                    lor=1./1.e-4
!                    h=1.d0+gamma/(gamma-1.d0)*qp/qd
                    
!                    qx = Mx/M*(lor**2-1.d0)**0.5/lor
!                    qy = My/M*(lor**2-1.d0)**0.5/lor
!                    qz = Mz/M*(lor**2-1.d0)**0.5/lor
                    !
!                    uold(ind_cell_son(i),1)=qd*lor
!                    uold(ind_cell_son(i),2)=qd*lor**2*h*qx
!                    uold(ind_cell_son(i),3)=qd*lor**2*h*qy
!                    uold(ind_cell_son(i),4)=qd*lor**2*h*qz
!                    uold(ind_cell_son(i),5)=qd*lor**2*h - qp
  ...
  !                  E =uold(ind_cell_son(i),5) 
   ...
   !              endif
           ...
           do i=1,ncell
              ...
              uold(ind_cell(i),5)=uold(ind_cell(i),1)*h*lor-(gamma-1.0d0)*uold(ind_cell(i),1)/lor*getx(i)/dble(twotondim)
        ...
        do i=1,ncell
           if (uold(ind_cell(i),5) <0) then
              write(*,*),'father energy negative'
              write(*,*),uold(ind_cell(i),5),'Efat'
           endif
           
           if (uold(ind_cell(i),5) <0) then
              write(*,*),'father density negative'
              write(*,*),uold(ind_cell(i),1),'Dfat'
           endif
           Mx=uold(ind_cell(i),2) ; My=uold(ind_cell(i),3) ; Mz=uold(ind_cell(i),4)
           M = sqrt(Mx**2+My**2+Mz**2)
           if (uold(ind_cell(i),5)**2<(M**2+uold(ind_cell(i),1)**2)) then
              write(*,*),'Efat^2<M^2+D^2'
              write(*,*),uold(ind_cell(i),1),M,uold(ind_cell(i),5),'D,M,E,fat',i
           endif
        enddo
...
subroutine interpol_hydro(u1,g1,u2,g2,nn)
  ...
  if(interpol_var==1)then
     do j=0,twondim
        do i=1,nn
           ...
           !Method from Mignone,McKinney,2007. Same as BS2011 except one uses E'=U-D and u^2=Lor^2*v^2
           if (M>E) then
              write (*,*) 'M>E',i,D,M,E,'interpol_hydro prolo'
           endif
           if ((E**2<M**2+D**2).or.(E<0)) then
              
              write (*,*) 'Switch...interpol_hydro prolongation'
              ...
              u1(i,j,1)=qd*lor
              u1(i,j,2)=qd*lor**2*h*qx
              u1(i,j,3)=qd*lor**2*h*qy
              u1(i,j,4)=qd*lor**2*h*qz
              u1(i,j,5)=qd*lor**2*h - qp
  ...
  if(interpol_var==1)then
     do ind=1,twotondim
        do i=1,nn
           h=1+gamma*u2(i,ind,5)
           m2=u2(i,ind,2)**2+u2(i,ind,3)**2+u2(i,ind,4)**2
           lor=sqrt(m2/u2(i,ind,1)**2/h**2+1.0d0)
           u2(i,ind,5)=u2(i,ind,1)*h*lor-(gamma-1.0d0)*u2(i,ind,1)/lor*u2(i,ind,5)
        end do
     end do
  end if
