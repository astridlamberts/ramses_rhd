...
!     q(k,5)=(2d0*xsi*(xsi+2d0*rho))/(5d0*(xsi+rho)+sqrt(9d0*xsi**2+18d0*rho*xsi+25d0*rho**2))
     q(k,5)=((E-M*q(k,2))**2-rho**2)/(3d0*(E-M*q(k,2)))
  ...
  ! Passive scalar
     ...
     M2=lor**2*rho*h*q(k,2)
     E2=lor**2*rho*h-q(k,5)
!     if ((abs(D-d2) .gt. 1e-10 )) write(*,*),D,D2,'Dbis'
!     if ((abs(M-M2) .gt. 1e-10))  write (*,*)M,M2,'Mbis'
!     if ((abs(E-E2) .gt. 1e-10))  write(*,*),E,E2,'Ebis',q(k,1),q(k,2),q(k,5)
