...
!  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2)::c  
  ...
  real(dp)::half=0.5
  real(dp)::small_bigD=1d-12,rho,D2,M2,E2,h,tau,p
...
!             p=(-5d0*(rho+xsi)+sqrt(25d0*(rho*xsi)**2+16*xsi*(xsi+2d0*rho)))/(-8d0)
             p=((E-M*q(l,i,j,k,2))**2-rho**2)/(3d0*(E-M*q(l,i,j,k,2)))
             q(l,i,j,k,5)=p
...
!             lor=(1.-(q(i,2)**2+q(i,3)**2+q(i,4)**2))**(-1.d0/2.d0)
             ...
             M2=lor**2*rho*h*q(l,i,j,k,2)
             E2=lor**2*rho*h-q(l,i,j,k,5)
             if ((abs(D-D2) .gt. 1e-10 )) write(*,*),D,D2,'D'
             if ((abs(M-M2) .gt. 1e-10))  write (*,*)M,M2,'M'
             if ((abs(E-E2) .gt. 1e-10))  write(*,*),E,E2,'E',q(l,i,j,k,1),q(l,i,j,k,2),q(l,i,j,k,5),p
...
function f_prim_Mignone(R,D,M,Eprim,gamma)
  ...
  real(dp)::u2,lor,dpdxsi,rho,xsi,dpdrho,dv2dR,dxsidR,drhodR,P,dPdR
  u2=M**2/((R+D)**2-M**2) ; lor=(1+u2)**(0.5)
  Xsi=(R-u2/(lor+1)*D)/lor**2
  rho=D/lor
  P=(2d0*xsi*(xsi+2d0*rho))/(5d0*(xsi+rho)+sqrt(9d0*xsi**2+18d0*rho*xsi+25d0*rho**2))

  dpdxsi=(2d0*xsi+2d0*xsi-5d0*P)/(5d0*rho+5d0*xsi-8d0*P)
