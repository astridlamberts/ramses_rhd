...
subroutine upl(ind_cell,ncell)
  ...
  real(dp)::D,Mx,My,Mz,M,E,M2, lor, h,xsi,qd,qp,qx,qy,qz,r,v2,tau,emean
...
!                 write(*,*),'switch'
                 ...
                 tau=qp/qd
                 eint(i)=3d0/2d0*tau-1+sqrt(9d0/4d0*tau**2+1) 
              end do
           ...
           do i=1,ncell
              emean=getx(i)/dble(twotondim)
              tau=(emean*(emean+2))/(emean+1d0)/3d0
              h=1.0d0+emean+tau
...
subroutine interpol_hydro(u1,g1,u2,g2,nn)
  ...
  real(dp) ::qp,qd,qx,qy,qz,h,m2,tau,emean
  real(dp) :: small_bigD=1e-12
     ...
     do j=0,twondim
        do i=1,nn
           ...
           if ( M .eq. 0) then 
             ...
             tau=qp/qd
             u1(i,j,5)=3d0/2d0*tau-1+sqrt(9d0/4d0*tau**2+1) 
