...
implicit none
  ...
  do k=1,n
     ...
     D2= q(k,1)*lor
     M2=lor**2*q(k,1)*h*sqrt(q(k,2)**2+q(k,3)**2+q(k,4)**2)
     E2=lor**2*q(k,1)*h-q(k,5)
!     if ((abs(D-d2)/D2 .gt. 1e-5 )) write(*,*),D,D2,'Dbis'
!     if ((abs(M-M2)/M2 .gt. 1e-5))  write (*,*)M,M2,'Mbis'
!     if ((abs(E-E2)/E2 .gt. 1e-5))  write(*,*),E,E2,'Ebis',q(k,1),q(k,2),q(k,5)
