...
subroutine find_mhd_flux(qvar,cvar,ff)
  ...
  real(dp)                    :: ecin,etot,d,u,v,w,p,entho,ein,enth,tau
...
!  enth = ein + p !relativistic enthalpy
  ...
  cvar(1) = d*lor
  cvar(2) = d*enth*lor**2-p
  cvar(3) = d*enth*lor**2*u
  cvar(4) = d*enth*lor**2*v
  cvar(5) = d*enth*lor**2*w
  do ivar = 6,nvar  
  ...
  ff(1) = d*u*lor
  ff(2) = d*enth*lor**2*u
  ff(3) = d*enth*lor**2*u**2+p
  ff(4) = d*enth*lor**2*u*v
  ff(5) = d*enth*lor**2*u*w
  do ivar =6,nvar
...
implicit none
  ...
  integer::n,k,idim
  real(dp) :: lor,entho ! Lorentz factor
  real(dp) :: D,M,E,Mx,My,Mz,u2,Xsi,R,D2,m2,E2
  real(dp) :: small_bigD=1e-12,rho,tau,h
  ...
  do k=1,n
     ...
     if ( M .eq. 0) then 
        ...
        ! compute velocities
        q(k,2) = Mx/R!max(1.0e-10,Mx/R)
        q(k,3) = My/R!max(1.0e-10,My/R)
        q(k,4) = Mz/R!max(1.0e-10,Mz/R)
     ...
     M2=lor**2*q(k,1)*h*q(k,2)
     E2=lor**2*q(k,1)*h-q(k,5)
     if ((abs(D-d2) .gt. 1e-5 )) write(*,*),D,D2,'Dbis'
     if ((abs(M-M2) .gt. 1e-5))  write (*,*)M,M2,'Mbis'
     if ((abs(E-E2) .gt. 1e-5))  write(*,*),E,E2,'Ebis',q(k,1),q(k,2),q(k,5)
