...
subroutine condinit(x,u,dx,nn)
  ...
  real(dp):: rho_amb,P_amb,v_a,P_a,rho_a1,rho_a2,h,lor,lor1,lor2,P_a1,P_a2,tau
  integer :: ivar,ncell
  real(dp),dimension(1:nvector,1:nvar)::q ! Primitive variables
  real(dp)::a1=1.0d0 ! separation of the system in simulation units
  integer::index_pas_scal1=0
  integer::index_pas_scal2=0
...
! only the stellar wind has a passive scalar  
  if (type1 == 'star') then 
     index_pas_scal1=1
  else 
     if (type2 == 'star') then
     ...
     call init_star(q,x,nn,vinf1,r01,Mdot1,Mach1,x01,y01,z01,index_pas_scal1)
...
!  if (type2 == 'star') then 
     call init_star(q,x,nn,vinf2,r02,Mdot2,Mach2,x02,y02,z02,index_pas_scal2)
...
!     if ((u(i,6) .ne. 0) .or. (u(i,7) .ne. 0)) then
!     write(*,*),u(i,6),u(i,7),i,'scalr ini'
!     endif
!     if (q(i,3) .ne. 0) then
...
subroutine init_star(q,x,nn,vinf,r0,Mdot,Mach,x0,y0,z0,index_pas_scal) 
  use wind_parameters
  use amr_parameters
  use hydro_parameters
  implicit none
...
! check if 3D velocity is ok!!!!
  ...
  DO i=1,nn 
     ...
     IF (rc .lt. r0) THEN
        ...
        if ((nvar .gt. 5) .and. (index_pas_scal .ne.0)) then      
...
! get size of cell
     ...
     ! dx_loc,utile?????
        ...
        call init2_star(xx,u_mask,dx_loc,ngrid,x01,y01,z01,r01,Mdot1,vinf1,Mach1,dx,index_pas_scal1)

        call init2_star(xx,u_mask,dx_loc,ngrid,x02,y02,z02,r02,Mdot2,vinf2,Mach2,dx,index_pas_scal2)
...
!         write(*,*),uold(ind_cell(i),1),uold(ind_cell(i),2),uold(ind_cell(i),5),rc1,rc2
         if (rc1 .lt. r01) then
  !             write(*,*),uold(ind_cell(i),1),uold(ind_cell(i),2),uold(ind_cell(i),5),'one'
               call reset_star(xc1,yc1,zc1,r01,dx,u_mask,i,ind_cell(i))

!               write(*,*),uold(ind_cell(i),6),xc1,yc1
!               write(*,*),u_mask(i,:),'one'
!               write(*,*)''
!               write(*,*)''
         else if (rc2 .lt. r02)then 
 !           write(*,*),uold(ind_cell(i),1),uold(ind_cell(i),2),uold(ind_cell(i),5),'two'
               call reset_star(xc2,yc2,zc2,r02,dx,u_mask,i,ind_cell(i))

!               write(*,*),u_mask(i,:),'two'
 !        else
!            write(*,*),uold(ind_cell(i),1),uold(ind_cell(i),2),uold(ind_cell(i),5),'none'
         endif
         
         if (nvar  .gt. 5) then 
            if (uold(ind_cell(i),6)/uold(ind_cell(i),1) .lt. 1.0e-4 )then 
               uold(ind_cell(i),6)=0
           endif
         endif
...
subroutine reset_star(xc,yc,zc,r0,delta_x,u_mask,i,ind_cell)              
!routine which changes re-initializes the values of the hydro-variables in the mask zone. 
...
subroutine init2_star(x,u,dx,nn,x0,y0,z0,r0,Mdot,vinf,Mach,delta_x,index_pas_scal)
...
!  if ((vinf .eq. 0.01) .and. (index_pas_scal .ne. 1)) then
!     write(*,*),vinf,index_pas_scal,'passcal'
!  endif
...
!  write(*,*),index_pas_scal,'index'
  ...
  DO i=1,nn ! all the cells
     ...
     IF (rc .lt. r0) THEN
        ...
        !passive scalars
        if (index_pas_scal .ne. 0) then
           u(:,5+index_pas_scal)=u(:,1)*q(:,5+index_pas_scal)!q(1:nn,1)*q(1:nn,ivar)*lor
        endif
!           if (vinf < .4) then
!              write(*,*),u(i,6),q(i,6),i,index_pas_scal
!          endif
!        end do
!        if ((u(i,6) .ne. 0) .or. (u(i,7) .ne. 0)) then
!        write(*,*),u(i,7),q(i,1),q(i,7),lor,i,'scal2'
!        endif
     ENDIF
   ENDDO 
...
real(kind=8)::ka,a,b,c,delta,p1,int2,lor,d,e
...
!         if ((vinf .lt. .1) .or. (eos .ne. 'TM'))then
            int =int +(dx/n)**ndim*P_a*(min((a1/r)**(gamma),10*(a1/r0)**gamma))
!         else
            
!            lor=(1d0-vinf**2)**(-1./2.)
!            ka=a1*(5d0/2d0*P_a+sqrt(9d0/4d0*P_a**2+rho_a**2))*lor**2*vinf
!            a=-4d0!*lor**4*vinf**2
!            b=5d0*ka/r
!            c=-ka**2/r**2+rho_r**2!*lor**4*vinf**2

  !         d=b**2
!            e=4d0*a*c
!            delta=d-e
!            p1=(-b+sqrt(delta))/2d0/a
!            int=int+(dx/n)**ndim*min(p1,10*(a1/r0)**gamma)
!         endif
!         write(*,*),r,a,b,c,delta,sqrt(delta),P1,'i'
!         write(*,*),sqrt(delta),-b,r,vinf,'i'
!         write(*,*),delta
      end do
   end do
#endif
