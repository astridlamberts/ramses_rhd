...
#ifndef WITHOUTMPI
  call MPI_ALLREDUCE(ok_free,ok_all,1,MPI_LOGICAL,MPI_LAND,MPI_COMM_WORLD,info)
#endif
#ifdef WITHOUTMPI
  ok_all=ok_free
#endif
...
subroutine make_grid_coarse(ind_cell,ibound,boundary_region)
  ...
  if(boundary_region)then
     ...
     do j=1,twotondim
        ...
        do idim=1,ndim
           xx(1,idim)=(xx(1,idim)-skip_loc(idim))*boxlen/dble(nx_loc)
        end do
...
subroutine refine_fine(ilevel)
  ...
  !---------------------------------
  ! Count cells that will be refined
  !---------------------------------
  ncreate=0
...
subroutine make_grid_fine(ind_grid,ind_cell,ind,ilevel,nn,ibound,boundary_region)
  ...
  if(.not.init .and. hydro)then
     ...
     ! Interpolate
     if(nn>0)call interpol_hydro(u1,ind1,u2,nn)
     ...
     do j=1,twotondim
        ...
        if(nn>0)call velana(xx,vv,dx_loc,t,nn)
        iskip=ncoarse+(j-1)*ngridmax
        do idim=1,ndim
           do i=1,nn
              v(iskip+ind_grid_son(i),idim)=vv(i,idim)
           end do
        end do
     end do
  end if
