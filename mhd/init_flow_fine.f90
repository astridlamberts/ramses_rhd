  ...
  if(ok_file)then
     ...
     do ivar=1,nvar_in
        if(cosmo)then
           ...
           if(ivar==9)filename=TRIM(initfile(ilevel))//'/ic_bxright'
           if(ivar==10)filename=TRIM(initfile(ilevel))//'/ic_byright'
           if(ivar==11)filename=TRIM(initfile(ilevel))//'/ic_bzright'
        endif
        ...
        do ind=1,twotondim
           ...
           do i=1,ncache
              ...
              if(ivar<9)then
                 ...
                 uold(icell,nvar+ivar-8)=init_array(i1,i2,i3)
              endif
           end do
        end do
        ! End loop over cells
     ...
     do igrid=1,ncache,nvector
        ...
        do ind=1,twotondim
           ...
           ! Compute metal mass density
           if(metal)then
              do i=1,ngrid
                 rr=uold(ind_cell(i),1)
                 uold(ind_cell(i),6)=rr*uold(ind_cell(i),6)
              end do
           endif
           ! Compute momentum
           do ivar=1,ndim
