subroutine write_screen
  ...
  real(dp)::dx,scale,smallp,ddd,ppp,ttt

  integer     ,dimension(:),allocatable::ind_grid,ind_cell,ind_sort,ll,ll_all
  real(kind=8),dimension(:),allocatable::rr,et,ei,dd,uu,mm,gg,dtot,tt
  real(kind=8),dimension(:),allocatable::rr_all,et_all,ei_all
  real(kind=8),dimension(:),allocatable::dd_all,uu_all,mm_all,gg_all,dtot_all,tt_all
  ...
  allocate(rr(1:ncell),mm(1:ncell),dd(1:ncell),dtot(1:ncell),tt(1:ncell))
  allocate(et(1:ncell),ei(1:ncell))
  allocate(uu(1:ncell),ll(1:ncell),gg(1:ncell))
  allocate(rr_all(1:ncell),mm_all(1:ncell),dd_all(1:ncell),dtot_all(1:ncell),tt_all(1:ncell))
  allocate(et_all(1:ncell),ei_all(1:ncell))
  allocate(uu_all(1:ncell),ll_all(1:ncell),gg_all(1:ncell))
  rr=0.0D0; mm=0.0D0; dd=0.0D0; dtot=0.0D0; et=0.0D0; tt=0.0D0
  ei=0.0D0; uu=0.0D0; gg=0.0D0; ll=0
  rr_all=0.0D0; mm_all=0.0D0; dd_all=0.0D0; dtot_all=0.0D0; et_all=0.0D0
  ei_all=0.0D0; uu_all=0.0D0; gg_all=0.0D0; ll_all=0; tt_all=0.0D0
           ...
           do ind=1,twotondim
              ...
              do i=1,ncache
                 if(son(ind_cell(i))==0)then
                    ...
                    tt(icell)=uold(ind_cell(i),ndim+3)/dd(icell)
                 end if
              end do
           end do
        end if
  ...
  call MPI_ALLREDUCE(rr,rr_all,ncell,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,info)
  call MPI_ALLREDUCE(tt,tt_all,ncell,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,info)
  rr=rr_all; mm=mm_all; dd=dd_all; dtot=dtot_all; et=et_all
  ei=ei_all; uu=uu_all; gg=gg_all; ll=ll_all; tt=tt_all
#endif

  if(myid==1)then
     write(*,*)'========================================================' 
     write(*,*)'lev      x           d          u          P          Te' 
     ! Sort radius
...
!!$          & gg(ind_sort(i))
  ...
  deallocate(mm,rr,dd,dtot,et,ei,uu,ll,gg,tt)
  deallocate(mm_all,rr_all,dd_all,dtot_all,et_all,ei_all,uu_all,ll_all,gg_all,tt_all)
