subroutine courant_fine(ilevel)
  ...
  !==========================================
  ! Conduction courant condition
  ! To estimate dtcond_init, compute:
  ! dtcond_init=0.8*(dx*scale)^2*d/(2*(gamma-1)*lambda)
  !==========================================
  if(diffusion.and.dt_control) then
     if(dtexpl_params(1)*dtexpl_params(2)**nstep_coarse.gt.1.0d14) then 
        dt_control=.false.
     else
     dtnew(ilevel)=MIN(dtnew(ilevel),&
          & dtexpl_params(1)*dtexpl_params(2)**nstep_coarse)
     end if
  end if
