...
subroutine set_uold(ilevel)
  ...
  do ind=1,twotondim
    ...
    if(diffusion)then
       do i=1,active(ilevel)%ngrid
          unew(active(ilevel)%igrid(i)+iskip,ndim+3)=unew(active(ilevel)%igrid(i)+iskip,ndim+3) &
               & +(gamma-1.0)*uold(active(ilevel)%igrid(i)+iskip,ndim+3)*divu(active(ilevel)%igrid(i)+iskip)
       end do
    end if
     do ivar=1,nvar
        do i=1,active(ilevel)%ngrid
     ...
     if(pressure_fix)then
        ...
        do i=1,active(ilevel)%ngrid
           ...
           e_trunc=0.0*d*max(div,3.0*hexp*dx)**2
