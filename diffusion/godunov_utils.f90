...
subroutine riemann_approx(qleft,qright,qgdnv,fgdnv,ngrid)
  ...
  real(dp),dimension(1:nvector),save::el, er, eo, estar
  ...
  do i=1,ngrid
     if(pstar(i)>= po(i))then
        ...
        estar(i) = eo(i)*(rstar(i)/ro(i))**(gamma)
     else
        ! Rarefaction
        rstar(i) = ro(i)*(pstar(i)/po(i))**(one/gamma)
        estar(i) = eo(i)*(pstar(i)/po(i))
     end if
  end do
  ...
  do i=1,ngrid
     if(spout(i)<=zero)then
        ...
        qgdnv(i,4) = frac(i)*estar(i) + (one - frac(i))*eo(i)
...
!!$     pstar(i) = MAX(pstar(i),zero)
  ...
  do i=1,ngrid
     ...
     cstar(i) = sqrt(abs(gamma*pstar(i)/rstar(i)))
     cstar(i) = max(cstar(i),smallc)
     estar(i) = eo(i) + eo(i)*(pstar(i)-po(i))/po(i)
  end do
