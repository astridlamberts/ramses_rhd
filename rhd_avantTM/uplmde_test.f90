     ...
     do j = jlo, jhi
        do i = ilo, ihi
           do l = 1, ngrid
              ...
              rm=v*cc/lor/r*(-r*cc-u*cc*(csq-1))/(u*csq-cc*eta)
              rp=v*cc/lor/r*(-r*cc-u*cc*(csq-1))/(u*csq+cc*eta)
 !             write(*,*),rp,rm,'real'
              ...
              a32= v*r*chi/f
              a33= u
              a35= -(u*v*(-h+r*chi+r*h*kappa))/(r*h)/f

!              spminus = (u*(one-ccc**2)-ccc*sqrt(coef))/(one-vtot2*ccc**2)
!              spplus  = (u*(one-ccc**2)+ccc*sqrt(coef))/(one-vtot2*ccc**2)

!              rm=(-a*a32+b*a35)/(spminus/dtdx-a33)
!              rp=( a*a32+b*a35)/(spplus/dtdx-a33)
!              write(*,*),rm,rp,'x'
...
!              rp=-cc*(lor*eta*u+cc)/(lor**2*r*(1d0-u**2))
!              rm= cc*(lor*eta*u-cc)/(lor**2*r*(1d0-u**2))
...
!              rp=v*cc/lor/r*(-r*cc-u*cc*(csq-1))/(u*csq+cc*eta)
 ...
 !              write(*,*),rp,rm,'moi'
              ...
              qp(l,i,j,k,3,1) = v + (amright*rm*v + azryright + apright*rp*v)
              qp(l,i,j,k,4,1) = w + (amright*rm*w + azrzright + apright*rp*w)
              ...
              qm(l,i,j,k,3,1) = v + (amleft*rm + azryleft + apleft*rp)
              qm(l,i,j,k,4,1) = w + (rm*amleft + azrzleft + apleft*rp)
...
subroutine tracexy(q,dq,c,qm,qp,dx,dy,dt,ngrid)
  ...
  real(dp)    :: b22,b23,b25
  real(dp)    :: kappa,chi,f,cs2,entho,lorsq
...
!              rp=cc*rp/(r*(v*cc-lor*eta))
              ...
              b22= v
              b23= u*r*chi/f
              b25=-(v*u*(-h+r*chi+r*h*kappa))/(r*h)/f

!              spminus = (v*(one-ccc**2)-ccc*sqrt(coef))/(one-vtot2*ccc**2)
!              spplus  = (v*(one-ccc**2)+ccc*sqrt(coef))/(one-vtot2*ccc**2)


!              rm=(-a*b23+b*b25)/(spminus/dtdx-b22)
!              rp=( a*b23+b*b25)/(spplus/dtdx-b22)
!              write(*,*),rm,rp,'y'

              qp(l,i,j,k,1,2) = r + (apright+ azryright+amright)
              qp(l,i,j,k,2,2) = u + (amright*rm*u+ azrxright + apright*rp*u)
              qp(l,i,j,k,3,2) = v + (-amright + apright)*a
              qp(l,i,j,k,4,2) = w + (amright*rm*w + azrzright + apright*rp*w)
              qp(l,i,j,k,5,2) = p + ( amright+apright)*b
!              
              qp(l,i,j,k,ir,2) = max(smallr, qp(l,i,j,k,ir,2))
              qp(l,i,j,k,ip,2) = max(smallp, qp(l,i,j,k,ip,2))

!              lor=(one-qp(l,i,j,k,2,2)**2-qp(l,i,j,k,3,2)**2-qp(l,i,j,k,4,2)**2)**(-1./2.)
...
!              endif
              ...
              qm(l,i,j,k,2,2) = u + (amleft*rm+ azrxleft + apleft*rp)
              qm(l,i,j,k,3,2) = v + (-amleft + apleft)*a
              qm(l,i,j,k,4,2) = w + (amleft*rm + azrzleft + apleft*rp)
